...
router.post('/comunidad/:comunidadId', [authenticate, requireCommunity('comunidadId', ['admin']), body('rut').notEmpty(), body('dv').notEmpty(), body('razon_social').notEmpty()], async (req, res) => {
  const errors = validationResult(req); if (!errors.isEmpty()) return res.status(400).json({ errors: errors.array() });
  const comunidadId = Number(req.params.comunidadId); const { rut,dv,razon_social,giro,email,telefono,direccion } = req.body;
