...
// reservas (crear reserva requiere pertenecer a comunidad)
...
router.post('/:id/reservas', [authenticate, body('unidad_id').isInt(), body('persona_id').isInt(), body('inicio').notEmpty(), body('fin').notEmpty()], async (req, res) => {
  const id = req.params.id; const { unidad_id, persona_id, inicio, fin } = req.body;
  try { const [result] = await db.query('INSERT INTO reserva_amenidad (comunidad_id, amenidad_id, unidad_id, persona_id, inicio, fin) SELECT comunidad_id, ?, ?, ?, ?, ? FROM amenidad WHERE id = ?',[id, id, unidad_id, persona_id, inicio, fin, id]); const [row] = await db.query('SELECT id, inicio, fin, estado FROM reserva_amenidad WHERE id = ? LIMIT 1', [result.insertId]); res.status(201).json(row[0]); } catch (err) { console.error(err); res.status(500).json({ error: 'server error' }); }
});
