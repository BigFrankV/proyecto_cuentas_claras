...
/**
 * @openapi
 * /comunidades/{comunidadId}/documentos-compra:
 *   get:
...
router.post('/comunidad/:comunidadId', [authenticate, authorize('admin','superadmin'), body('proveedor_id').isInt(), body('tipo_doc').notEmpty(), body('folio').notEmpty(), body('fecha_emision').notEmpty(), body('total').isNumeric()], async (req, res) => {
  const errors = validationResult(req); if (!errors.isEmpty()) return res.status(400).json({ errors: errors.array() });
  const comunidadId = req.params.comunidadId; const { proveedor_id, tipo_doc, folio, fecha_emision, neto=0, iva=0, exento=0, total, glosa } = req.body;
