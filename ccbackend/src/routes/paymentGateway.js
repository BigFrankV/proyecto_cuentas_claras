...
router.post(
  ...
  [
    ...
    body('gateway')
      .isIn(['webpay', 'khipu', 'mercadopago'])
      .withMessage('Pasarela no válida'),
  ...
  async (req, res) => {
    try {
      ...
      const { communityId, unitId, amount, gateway, description, payerEmail } =
        req.body;
      ...
      const orderId = paymentGatewayService.generateOrderId(
        communityId,
        unitId
      );
 ...
 */
router.get(
  '/community/:communityId/transactions',
