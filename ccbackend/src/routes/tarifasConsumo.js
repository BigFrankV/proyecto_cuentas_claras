 ...
 */
// listar tarifas por comunidad
router.get('/comunidad/:comunidadId', authenticate, async (req, res) => { const comunidadId = req.params.comunidadId; const [rows] = await db.query('SELECT id, tipo, periodo_desde, precio_por_unidad, cargo_fijo FROM tarifa_consumo WHERE comunidad_id = ? ORDER BY periodo_desde DESC LIMIT 200', [comunidadId]); res.json(rows); });

router.post('/comunidad/:comunidadId', [authenticate, authorize('admin','superadmin'), body('tipo').notEmpty(), body('periodo_desde').notEmpty(), body('precio_por_unidad').isNumeric()], async (req, res) => { const comunidadId = req.params.comunidadId; const { tipo, periodo_desde, periodo_hasta, precio_por_unidad, cargo_fijo } = req.body; try { const [result] = await db.query('INSERT INTO tarifa_consumo (comunidad_id, tipo, periodo_desde, periodo_hasta, precio_por_unidad, cargo_fijo) VALUES (?,?,?,?,?,?)', [comunidadId, tipo, periodo_desde, periodo_hasta || null, precio_por_unidad, cargo_fijo || 0]); const [row] = await db.query('SELECT id, tipo, periodo_desde, precio_por_unidad FROM tarifa_consumo WHERE id = ? LIMIT 1', [result.insertId]); res.status(201).json(row[0]); } catch (err) { console.error(err); res.status(500).json({ error: 'server error' }); } });
