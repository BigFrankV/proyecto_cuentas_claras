...
router.get('/comunidad/:comunidadId/tickets', authenticate, requireCommunity('comunidadId'), async (req, res) => {
  ...
  const [rows] = await db.query('SELECT id, categoria, titulo, estado, prioridad FROM solicitud_soporte WHERE comunidad_id = ? ORDER BY created_at DESC LIMIT 200', [comunidadId]);
  res.json(rows);
});
...
router.post('/comunidad/:comunidadId/tickets', [authenticate, requireCommunity('comunidadId'), body('categoria').notEmpty(), body('titulo').notEmpty()], async (req, res) => {
  ...
  try {
    const [result] = await db.query('INSERT INTO solicitud_soporte (comunidad_id, unidad_id, categoria, titulo, descripcion, prioridad) VALUES (?,?,?,?,?,?)', [comunidadId, unidad_id || null, categoria, titulo, descripcion || null, prioridad || 'media']);
    const [row] = await db.query('SELECT id, categoria, titulo, estado FROM solicitud_soporte WHERE id = ? LIMIT 1', [result.insertId]);
    res.status(201).json(row[0]);
  } catch (err) { console.error(err); res.status(500).json({ error: 'server error' }); }
});

router.get('/tickets/:id', authenticate, async (req, res) => { const id = req.params.id; const [rows] = await db.query('SELECT * FROM solicitud_soporte WHERE id = ? LIMIT 1', [id]); if (!rows.length) return res.status(404).json({ error: 'not found' }); res.json(rows[0]); });
...
router.patch('/tickets/:id', authenticate, authorize('admin','superadmin'), async (req, res) => {
  ...
  try { await db.query(`UPDATE solicitud_soporte SET ${updates.join(', ')} WHERE id = ?`, values); const [rows] = await db.query('SELECT id, categoria, titulo, estado FROM solicitud_soporte WHERE id = ? LIMIT 1',[id]); res.json(rows[0]); } catch (err) { console.error(err); res.status(500).json({ error: 'server error' }); }
});
...
router.get('/comunidad/:comunidadId/bitacora', authenticate, requireCommunity('comunidadId'), async (req, res) => { const comunidadId = Number(req.params.comunidadId); const [rows] = await db.query('SELECT id, fecha_hora, evento, detalle FROM registro_conserjeria WHERE comunidad_id = ? ORDER BY fecha_hora DESC LIMIT 500', [comunidadId]); res.json(rows); });
...
router.post('/comunidad/:comunidadId/bitacora', [authenticate, requireCommunity('comunidadId', ['conserje','admin']), body('evento').notEmpty(), body('fecha_hora').notEmpty()], async (req, res) => {
  ...
  try { const [result] = await db.query('INSERT INTO registro_conserjeria (comunidad_id, fecha_hora, usuario_id, evento, detalle) VALUES (?,?,?,?,?)', [comunidadId, fecha_hora, usuario_id || null, evento, detalle || null]); const [row] = await db.query('SELECT id, fecha_hora, evento FROM registro_conserjeria WHERE id = ? LIMIT 1', [result.insertId]); res.status(201).json(row[0]); } catch (err) { console.error(err); res.status(500).json({ error: 'server error' }); }
});
