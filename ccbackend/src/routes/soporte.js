...
router.get(
  '/comunidad/:comunidadId/tickets',
  authenticate,
  requireCommunity('comunidadId'),
  async (req, res) => {
    const comunidadId = Number(req.params.comunidadId);
    const [rows] = await db.query(
      'SELECT id, categoria, titulo, estado, prioridad FROM ticket_soporte WHERE comunidad_id = ? ORDER BY created_at DESC LIMIT 200',
      [comunidadId]
    );
    res.json(rows);
  }
  ...
  async (req, res) => {
    ...
    try {
      const [result] = await db.query(
        'INSERT INTO ticket_soporte (comunidad_id, unidad_id, categoria, titulo, descripcion, prioridad) VALUES (?,?,?,?,?,?)',
        [
          comunidadId,
          unidad_id || null,
          categoria,
      ...
      const [row] = await db.query(
        'SELECT id, categoria, titulo, estado FROM ticket_soporte WHERE id = ? LIMIT 1',
        [result.insertId]
      );
...
router.get(
  ...
  async (req, res) => {
    const comunidadId = Number(req.params.comunidadId);
    const [rows] = await db.query(
      'SELECT id, tipo, titulo, mensaje, leida FROM notificacion_usuario WHERE comunidad_id = ? ORDER BY fecha_creacion DESC LIMIT 200',
      [comunidadId]
    );
    res.json(rows);
  }
...
router.post(
  ...
  [
    authenticate,
    requireCommunity('comunidadId', ['admin', 'conserje']),
    body('tipo').notEmpty(),
    body('titulo').notEmpty(),
  ...
  async (req, res) => {
    const comunidadId = Number(req.params.comunidadId);
    const { persona_id, tipo, titulo, mensaje, objeto_tipo, objeto_id } =
      req.body;
    try {
      const [result] = await db.query(
        'INSERT INTO notificacion_usuario (comunidad_id, persona_id, tipo, titulo, mensaje, objeto_tipo, objeto_id) VALUES (?,?,?,?,?,?,?)',
        [
          comunidadId,
          persona_id || null,
          tipo,
      ...
      const [row] = await db.query(
        'SELECT id, tipo, titulo, mensagem FROM notificacion_usuario WHERE id = ? LIMIT 1',
        [result.insertId]
      );
      res.status(201).json(row[0]);
    } catch (err) {
...
router.get(
  '/comunidad/:comunidadId/documentos',
  authenticate,
  requireCommunity('comunidadId'),
  async (req, res) => {
    const comunidadId = Number(req.params.comunidadId);
    const [rows] = await db.query(
      'SELECT id, tipo, titulo, url FROM documento_comunidad WHERE comunidad_id = ? ORDER BY created_at DESC LIMIT 200',
      [comunidadId]
    );
    res.json(rows);
  }
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      const [result] = await db.query(
        'INSERT INTO documento_comunidad (comunidad_id, tipo, titulo, url, periodo, visibilidad) VALUES (?,?,?,?,?,?)',
        [
          comunidadId,
          tipo,
          titulo,
      ...
      const [row] = await db.query(
        'SELECT id, tipo, titulo, url FROM documento_comunidad WHERE id = ? LIMIT 1',
        [result.insertId]
      );
      res.status(201).json(row[0]);
    } catch (err) {
...
router.delete(
  ...
  async (req, res) => {
    const id = req.params.id;
    try {
      await db.query('DELETE FROM documento_comunidad WHERE id = ?', [id]);
      res.status(204).end();
    } catch (err) {
      console.error(err);
      res.status(500).json({ error: 'server error' });
...
router.get(
  '/comunidad/:comunidadId/bitacora',
  authenticate,
  requireCommunity('comunidadId'),
  async (req, res) => {
    const comunidadId = Number(req.params.comunidadId);
    const [rows] = await db.query(
      'SELECT id, fecha_hora, evento, detalle FROM bitacora_conserjeria WHERE comunidad_id = ? ORDER BY fecha_hora DESC LIMIT 500',
      [comunidadId]
    );
    res.json(rows);
  }
...
router.post(
  ...
  [
    authenticate,
    requireCommunity('comunidadId', ['conserje', 'admin']),
    body('evento').notEmpty(),
    body('fecha_hora').notEmpty(),
  ],
  async (req, res) => {
    const comunidadId = Number(req.params.comunidadId);
    const { fecha_hora, usuario_id, evento, detalle } = req.body;
    try {
      const [result] = await db.query(
        'INSERT INTO bitacora_conserjeria (comunidad_id, fecha_hora, usuario_id, evento, detalle) VALUES (?,?,?,?,?)',
        [comunidadId, fecha_hora, usuario_id || null, evento, detalle || null]
      );
      const [row] = await db.query(
        'SELECT id, fecha_hora, evento FROM bitacora_conserjeria WHERE id = ? LIMIT 1',
        [result.insertId]
      );
      res.status(201).json(row[0]);
    } catch (err) {
...
router.get(
  '/comunidad/:comunidadId/parametros-cobranza',
  authenticate,
  requireCommunity('comunidadId'),
  async (req, res) => {
    const comunidadId = Number(req.params.comunidadId);
    const [rows] = await db.query(
      'SELECT * FROM parametros_cobranza WHERE comunidad_id = ? LIMIT 1',
      [comunidadId]
    );
...
router.patch(
  '/comunidad/:comunidadId/parametros-cobranza',
  [authenticate, requireCommunity('comunidadId', ['admin', 'contador'])],
  async (req, res) => {
    const comunidadId = Number(req.params.comunidadId);
    const fields = [
      'dias_gracia',
    ...
    try {
      await db.query(
        `UPDATE parametros_cobranza SET ${updates.join(
          ', '
        )} WHERE comunidad_id = ?`,
        values
      );
