 ...
 *   get:
 *     tags: [Conciliaciones]
 *     summary: Listar conciliaciones bancarias por comunidad
 *     security:
 *       - bearerAuth: []
 *     parameters:
 *       - in: path
 ...
 */
router.get('/comunidad/:comunidadId', authenticate, async (req, res) => { const comunidadId = req.params.comunidadId; const [rows] = await db.query('SELECT id, fecha_mov, monto, estado FROM conciliacion_bancaria WHERE comunidad_id = ? ORDER BY fecha_mov DESC LIMIT 200', [comunidadId]); res.json(rows); });

router.post('/comunidad/:comunidadId', [authenticate, authorize('admin','superadmin'), body('fecha_mov').notEmpty(), body('monto').isNumeric()], async (req, res) => { const comunidadId = req.params.comunidadId; const { fecha_mov, monto, glosa, referencia, pago_id } = req.body; try { const [result] = await db.query('INSERT INTO conciliacion_bancaria (comunidad_id, fecha_mov, monto, glosa, referencia, pago_id) VALUES (?,?,?,?,?,?)', [comunidadId, fecha_mov, monto, glosa || null, referencia || null, pago_id || null]); const [row] = await db.query('SELECT id, fecha_mov, monto, estado FROM conciliacion_bancaria WHERE id = ? LIMIT 1', [result.insertId]); res.status(201).json(row[0]); } catch (err) { console.error(err); res.status(500).json({ error: 'server error' }); } });
