...
router.get('/:id/cuentas', authenticate, async (req, res) => {
  try {
    const id = req.params.id;
    const [rows] = await db.query(`SELECT ccu.id, ccu.emision_id, ccu.comunidad_id, ccu.monto_total, ccu.saldo, ccu.estado, ccu.interes_acumulado, ccu.created_at, ccu.updated_at, eg.periodo AS emision_periodo, eg.fecha_vencimiento AS emision_vencimiento FROM cuenta_cobro_unidad ccu LEFT JOIN emision_gastos_comunes eg ON eg.id = ccu.emision_id WHERE ccu.unidad_id = ? ORDER BY ccu.created_at DESC`, [id]);
    res.json(rows);
...
router.get('/cuentas/:cuentaId/detalle', authenticate, async (req, res) => {
  try {
    ...
    const [rows] = await db.query(`SELECT dcu.id, dcu.categoria_id, cat.nombre AS categoria_nombre, dcu.glosa, dcu.monto, dcu.origen, dcu.origen_id, dcu.iva_incluido, dcu.created_at FROM detalle_cuenta_unidad dcu LEFT JOIN categoria_gasto cat ON cat.id = dcu.categoria_id WHERE dcu.cuenta_cobro_unidad_id = ? ORDER BY dcu.id`, [cuentaId]);
    res.json(rows);
  } catch (err) { console.error(err); res.status(500).json({ error: 'server error' }); }
});
...
router.get('/:id/pagos', authenticate, async (req, res) => {
  try {
    const id = req.params.id;
    const [rows] = await db.query(`SELECT p.id, p.fecha, p.monto, p.medio, p.referencia, p.estado, p.comprobante_num, pa.cuenta_cobro_unidad_id, pa.monto AS aplicado_monto, pa.prioridad FROM pago p LEFT JOIN pago_aplicacion pa ON pa.pago_id = p.id WHERE p.unidad_id = ? ORDER BY p.fecha DESC, p.id DESC`, [id]);
    res.json(rows);
  } catch (err) { console.error(err); res.status(500).json({ error: 'server error' }); }
});
...
router.get('/cuentas/:cuentaId/aplicaciones', authenticate, async (req, res) => {
  try {
    const cuentaId = req.params.cuentaId;
    const [rows] = await db.query(`SELECT pa.id, pa.pago_id, pa.monto, pa.prioridad, p.fecha AS pago_fecha, p.monto AS pago_monto, p.medio, p.comprobante_num FROM pago_aplicacion pa LEFT JOIN pago p ON p.id = pa.pago_id WHERE pa.cuenta_cobro_unidad_id = ? ORDER BY pa.id`, [cuentaId]);
    res.json(rows);
  } catch (err) { console.error(err); res.status(500).json({ error: 'server error' }); }
});
...
router.get('/:id/financiero', authenticate, async (req, res) => {
  try {
    const id = req.params.id;
    const [rows] = await db.query('SELECT * FROM cargo_financiero_unidad v WHERE v.unidad_id = ? ORDER BY v.created_at DESC', [id]);
    res.json(rows);
  } catch (err) { console.error(err); res.status(500).json({ error: 'server error' }); }
});
...
router.get('/medidores/:medidorId/lecturas', authenticate, async (req, res) => {
  try {
    ...
    const [rows] = await db.query(sql, [medidorId, periodo_desde || null, periodo_desde || null, periodo_hasta || null, periodo_hasta || null]);
    res.json(rows);
  } catch (err) { console.error(err); res.status(500).json({ error: 'server error' }); }
});
...
router.get('/:id/tickets', authenticate, async (req, res) => {
  try {
    const id = req.params.id;
    const [rows] = await db.query('SELECT ts.id, ts.categoria, ts.titulo, ts.estado, ts.prioridad, ts.asignado_a, ts.created_at FROM ticket_soporte ts WHERE ts.unidad_id = ? ORDER BY ts.created_at DESC', [id]);
    res.json(rows);
  } catch (err) { console.error(err); res.status(500).json({ error: 'server error' }); }
});
...
router.get('/:id/multas', authenticate, async (req, res) => {
  try {
    const id = req.params.id;
    const [rows] = await db.query('SELECT id, motivo, descripcion, monto, persona_id, created_at, updated_at FROM multa WHERE unidad_id = ? ORDER BY created_at DESC', [id]);
    res.json(rows);
  } catch (err) { console.error(err); res.status(500).json({ error: 'server error' }); }
});
...
router.get('/:id/reservas', authenticate, async (req, res) => {
  try {
    const id = req.params.id;
    const [rows] = await db.query('SELECT id, amenidad_id, inicio, fin, estado, created_at FROM reserva_amenidad WHERE unidad_id = ? ORDER BY inicio DESC', [id]);
    res.json(rows);
  } catch (err) { console.error(err); res.status(500).json({ error: 'server error' }); }
});
...
router.get('/dropdowns/comunidades', authenticate, async (req, res) => {
  try { const [rows] = await db.query('SELECT id, razon_social AS nombre FROM comunidad ORDER BY nombre'); res.json(rows); } catch (err) { console.error(err); res.status(500).json({ error: 'server error' }); }
});
router.get('/dropdowns/edificios', authenticate, async (req, res) => { try { const { comunidad_id } = req.query; const [rows] = await db.query('SELECT id, nombre, direccion FROM edificio WHERE comunidad_id = ? ORDER BY nombre', [comunidad_id]); res.json(rows); } catch (err) { console.error(err); res.status(500).json({ error: 'server error' }); } });
router.get('/dropdowns/torres', authenticate, async (req, res) => { try { const { edificio_id } = req.query; const [rows] = await db.query('SELECT id, nombre, codigo FROM torre WHERE edificio_id = ? ORDER BY nombre', [edificio_id]); res.json(rows); } catch (err) { console.error(err); res.status(500).json({ error: 'server error' }); } });
router.get('/dropdowns/unidades', authenticate, async (req, res) => { try { const { torre_id } = req.query; const [rows] = await db.query('SELECT u.id, u.codigo AS numero, u.nro_estacionamiento, u.nro_bodega FROM unidad u WHERE u.torre_id = ? ORDER BY u.codigo', [torre_id]); res.json(rows); } catch (err) { console.error(err); res.status(500).json({ error: 'server error' }); } });
...
router.get('/search', authenticate, async (req, res) => {
  try {
    const q = req.query.q || '';
    const like = `%${q}%`;
    const [rows] = await db.query(`SELECT DISTINCT u.id, u.codigo, CONCAT(pe.nombres,' ',pe.apellidos) AS persona FROM unidad u JOIN titulares_unidad tu ON tu.unidad_id = u.id JOIN persona pe ON pe.id = tu.persona_id WHERE (pe.nombres LIKE ? OR pe.apellidos LIKE ? OR pe.rut LIKE ?) ORDER BY u.codigo LIMIT 50`, [like, like, like]);
    res.json(rows);
  } catch (err) { console.error(err); res.status(500).json({ error: 'server error' }); }
});
...
router.get('/validate/bodega', authenticate, async (req, res) => { try { const { comunidad_id, nro_bodega } = req.query; const [rows] = await db.query('SELECT id, codigo FROM unidad WHERE comunidad_id = ? AND nro_bodega = ? LIMIT 1', [comunidad_id, nro_bodega]); res.json(rows.length ? rows[0] : null); } catch (err) { console.error(err); res.status(500).json({ error: 'server error' }); } });
...
router.patch('/tenencias/:tenenciaId', [authenticate, authorize('admin','superadmin')], async (req, res) => {
  try {
    ...
    values.push(id);
    await db.query(`UPDATE titulares_unidad SET ${updates.join(', ')} WHERE id = ?`, values);
    const [rows] = await db.query('SELECT id, persona_id, tipo, desde, hasta, porcentaje FROM titulares_unidad WHERE id = ? LIMIT 1', [id]);
    res.json(rows[0]);
  } catch (err) { console.error(err); res.status(500).json({ error: 'server error' }); }
});

// Delete tenencia
router.delete('/tenencias/:tenenciaId', [authenticate, authorize('admin','superadmin')], async (req, res) => {
  try { const id = req.params.tenenciaId; await db.query('DELETE FROM titulares_unidad WHERE id = ?', [id]); res.status(204).end(); } catch (err) { console.error(err); res.status(500).json({ error: 'server error' }); }
});

// Create pago for unit
router.post('/:id/pagos', [authenticate, body('monto').isFloat({ gt: 0 })], async (req, res) => {
  const unidadId = req.params.id;
  const errors = validationResult(req); if (!errors.isEmpty()) return res.status(400).json({ errors: errors.array() });
  try {
    const { fecha, monto, medio, referencia, estado = 'pendiente', comprobante_num } = req.body;
    const [result] = await db.query('INSERT INTO pago (unidad_id, fecha, monto, medio, referencia, estado, comprobante_num) VALUES (?,?,?,?,?,?,?)', [unidadId, fecha || new Date(), monto, medio || null, referencia || null, estado, comprobante_num || null]);
    const [row] = await db.query('SELECT * FROM pago WHERE id = ? LIMIT 1', [result.insertId]);
    res.status(201).json(row[0]);
  } catch (err) { console.error(err); res.status(500).json({ error: 'server error' }); }
});

// Update pago
router.patch('/pagos/:pagoId', [authenticate, authorize('admin','superadmin')], async (req, res) => {
  try {
    const id = req.params.pagoId; const fields = ['fecha','monto','medio','referencia','estado','comprobante_num']; const updates = []; const values = [];
    fields.forEach(f => { if (req.body[f] !== undefined) { updates.push(`${f} = ?`); values.push(req.body[f]); } });
    if (!updates.length) return res.status(400).json({ error: 'no fields' }); values.push(id);
    await db.query(`UPDATE pago SET ${updates.join(', ')} WHERE id = ?`, values);
    const [rows] = await db.query('SELECT * FROM pago WHERE id = ? LIMIT 1', [id]); res.json(rows[0]);
  } catch (err) { console.error(err); res.status(500).json({ error: 'server error' }); }
});

// Delete pago
router.delete('/pagos/:pagoId', [authenticate, authorize('admin','superadmin')], async (req, res) => { try { const id = req.params.pagoId; await db.query('DELETE FROM pago WHERE id = ?', [id]); res.status(204).end(); } catch (err) { console.error(err); res.status(500).json({ error: 'server error' }); } });

// Create pago_aplicacion
router.post('/pagos/:pagoId/aplicaciones', [authenticate, authorize('admin','superadmin'), body('cuenta_cobro_unidad_id').isInt(), body('monto').isFloat({ gt: 0 })], async (req, res) => {
  const pagoId = req.params.pagoId; const errors = validationResult(req); if (!errors.isEmpty()) return res.status(400).json({ errors: errors.array() });
  try {
    const { cuenta_cobro_unidad_id, monto, prioridad = 1 } = req.body;
    const [result] = await db.query('INSERT INTO pago_aplicacion (pago_id, cuenta_cobro_unidad_id, monto, prioridad) VALUES (?,?,?,?)', [pagoId, cuenta_cobro_unidad_id, monto, prioridad]);
    const [row] = await db.query('SELECT * FROM pago_aplicacion WHERE id = ? LIMIT 1', [result.insertId]); res.status(201).json(row[0]);
  } catch (err) { console.error(err); res.status(500).json({ error: 'server error' }); }
});

// Delete pago_aplicacion
router.delete('/pago_aplicaciones/:id', [authenticate, authorize('admin','superadmin')], async (req, res) => { try { const id = req.params.id; await db.query('DELETE FROM pago_aplicacion WHERE id = ?', [id]); res.status(204).end(); } catch (err) { console.error(err); res.status(500).json({ error: 'server error' }); } });

// Medidor CRUD
router.post('/:id/medidores', [authenticate, authorize('admin','superadmin'), body('tipo').notEmpty()], async (req, res) => {
  try {
    const unidadId = req.params.id; const { tipo, codigo, es_compartido } = req.body;
    const [result] = await db.query('INSERT INTO medidor (unidad_id, tipo, codigo, es_compartido) VALUES (?,?,?,?)', [unidadId, tipo, codigo || null, es_compartido ? 1 : 0]);
    const [row] = await db.query('SELECT * FROM medidor WHERE id = ? LIMIT 1', [result.insertId]); res.status(201).json(row[0]);
  } catch (err) { console.error(err); res.status(500).json({ error: 'server error' }); }
});

router.patch('/medidores/:medidorId', [authenticate, authorize('admin','superadmin')], async (req, res) => { try { const id = req.params.medidorId; const fields = ['tipo','codigo','es_compartido','unidad_id']; const updates = []; const values = []; fields.forEach(f => { if (req.body[f] !== undefined) { updates.push(`${f} = ?`); values.push(req.body[f]); } }); if (!updates.length) return res.status(400).json({ error: 'no fields' }); values.push(id); await db.query(`UPDATE medidor SET ${updates.join(', ')} WHERE id = ?`, values); const [rows] = await db.query('SELECT * FROM medidor WHERE id = ? LIMIT 1', [id]); res.json(rows[0]); } catch (err) { console.error(err); res.status(500).json({ error: 'server error' }); } });

router.delete('/medidores/:medidorId', [authenticate, authorize('admin','superadmin')], async (req, res) => { try { const id = req.params.medidorId; await db.query('DELETE FROM medidor WHERE id = ?', [id]); res.status(204).end(); } catch (err) { console.error(err); res.status(500).json({ error: 'server error' }); } });

// Crear lectura
router.post('/medidores/:medidorId/lecturas', [authenticate, authorize('admin','superadmin'), body('lectura').isNumeric(), body('periodo').notEmpty()], async (req, res) => {
  const medidorId = req.params.medidorId; const errors = validationResult(req); if (!errors.isEmpty()) return res.status(400).json({ errors: errors.array() });
  try { const { periodo, fecha, lectura } = req.body; const [result] = await db.query('INSERT INTO lectura_medidor (medidor_id, periodo, fecha, lectura) VALUES (?,?,?,?)', [medidorId, periodo, fecha || new Date(), lectura]); const [row] = await db.query('SELECT * FROM lectura_medidor WHERE id = ? LIMIT 1', [result.insertId]); res.status(201).json(row[0]); } catch (err) { console.error(err); res.status(500).json({ error: 'server error' }); } });

// Multa CRUD
router.post('/:id/multas', [authenticate, authorize('admin','superadmin'), body('motivo').notEmpty(), body('monto').isFloat({ gt: 0 })], async (req, res) => { const unidadId = req.params.id; const errors = validationResult(req); if (!errors.isEmpty()) return res.status(400).json({ errors: errors.array() }); try { const { motivo, descripcion, monto, persona_id } = req.body; const [result] = await db.query('INSERT INTO multa (unidad_id, motivo, descripcion, monto, persona_id) VALUES (?,?,?,?,?)', [unidadId, motivo, descripcion || null, monto, persona_id || null]); const [row] = await db.query('SELECT * FROM multa WHERE id = ? LIMIT 1', [result.insertId]); res.status(201).json(row[0]); } catch (err) { console.error(err); res.status(500).json({ error: 'server error' }); } });

router.patch('/multas/:id', [authenticate, authorize('admin','superadmin')], async (req, res) => { try { const id = req.params.id; const fields = ['motivo','descripcion','monto','persona_id']; const updates = []; const values = []; fields.forEach(f => { if (req.body[f] !== undefined) { updates.push(`${f} = ?`); values.push(req.body[f]); } }); if (!updates.length) return res.status(400).json({ error: 'no fields' }); values.push(id); await db.query(`UPDATE multa SET ${updates.join(', ')} WHERE id = ?`, values); const [rows] = await db.query('SELECT * FROM multa WHERE id = ? LIMIT 1', [id]); res.json(rows[0]); } catch (err) { console.error(err); res.status(500).json({ error: 'server error' }); } });

router.delete('/multas/:id', [authenticate, authorize('admin','superadmin')], async (req, res) => { try { const id = req.params.id; await db.query('DELETE FROM multa WHERE id = ?', [id]); res.status(204).end(); } catch (err) { console.error(err); res.status(500).json({ error: 'server error' }); } });

// Reserva CRUD
router.post('/:id/reservas', [authenticate, body('amenidad_id').isInt(), body('inicio').notEmpty(), body('fin').notEmpty()], async (req, res) => { const unidadId = req.params.id; const errors = validationResult(req); if (!errors.isEmpty()) return res.status(400).json({ errors: errors.array() }); try { const { amenidad_id, inicio, fin, estado = 'pendiente' } = req.body; const [result] = await db.query('INSERT INTO reserva_amenidad (unidad_id, amenidad_id, inicio, fin, estado) VALUES (?,?,?,?,?)', [unidadId, amenidad_id, inicio, fin, estado]); const [row] = await db.query('SELECT * FROM reserva_amenidad WHERE id = ? LIMIT 1', [result.insertId]); res.status(201).json(row[0]); } catch (err) { console.error(err); res.status(500).json({ error: 'server error' }); } });

router.patch('/reservas/:id', [authenticate, authorize('admin','superadmin')], async (req, res) => { try { const id = req.params.id; const fields = ['amenidad_id','inicio','fin','estado']; const updates = []; const values = []; fields.forEach(f => { if (req.body[f] !== undefined) { updates.push(`${f} = ?`); values.push(req.body[f]); } }); if (!updates.length) return res.status(400).json({ error: 'no fields' }); values.push(id); await db.query(`UPDATE reserva_amenidad SET ${updates.join(', ')} WHERE id = ?`, values); const [rows] = await db.query('SELECT * FROM reserva_amenidad WHERE id = ? LIMIT 1', [id]); res.json(rows[0]); } catch (err) { console.error(err); res.status(500).json({ error: 'server error' }); } });

router.delete('/reservas/:id', [authenticate, authorize('admin','superadmin')], async (req, res) => { try { const id = req.params.id; await db.query('DELETE FROM reserva_amenidad WHERE id = ?', [id]); res.status(204).end(); } catch (err) { console.error(err); res.status(500).json({ error: 'server error' }); } });

// Ticket CRUD
router.post('/:id/tickets', [authenticate, body('categoria').notEmpty(), body('titulo').notEmpty()], async (req, res) => { const unidadId = req.params.id; const errors = validationResult(req); if (!errors.isEmpty()) return res.status(400).json({ errors: errors.array() }); try { const { categoria, titulo, descripcion, prioridad = 'media' } = req.body; const [result] = await db.query('INSERT INTO ticket_soporte (unidad_id, categoria, titulo, descripcion, prioridad, estado) VALUES (?,?,?,?,?,?)', [unidadId, categoria, titulo, descripcion || null, prioridad, 'abierto']); const [row] = await db.query('SELECT * FROM ticket_soporte WHERE id = ? LIMIT 1', [result.insertId]); res.status(201).json(row[0]); } catch (err) { console.error(err); res.status(500).json({ error: 'server error' }); } });

router.patch('/tickets/:id', [authenticate, authorize('admin','superadmin')], async (req, res) => { try { const id = req.params.id; const fields = ['categoria','titulo','descripcion','prioridad','estado','asignado_a']; const updates = []; const values = []; fields.forEach(f => { if (req.body[f] !== undefined) { updates.push(`${f} = ?`); values.push(req.body[f]); } }); if (!updates.length) return res.status(400).json({ error: 'no fields' }); values.push(id); await db.query(`UPDATE ticket_soporte SET ${updates.join(', ')} WHERE id = ?`, values); const [rows] = await db.query('SELECT * FROM ticket_soporte WHERE id = ? LIMIT 1', [id]); res.json(rows[0]); } catch (err) { console.error(err); res.status(500).json({ error: 'server error' }); } });

router.delete('/tickets/:id', [authenticate, authorize('admin','superadmin')], async (req, res) => { try { const id = req.params.id; await db.query('DELETE FROM ticket_soporte WHERE id = ?', [id]); res.status(204).end(); } catch (err) { console.error(err); res.status(500).json({ error: 'server error' }); } });
