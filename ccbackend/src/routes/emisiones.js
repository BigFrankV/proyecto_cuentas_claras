 ...
 * tags:
 *   - name: Emisiones
 *     description: Emisiones de gastos y detalles
 */
 ...
 *   get:
 *     tags: [Emisiones]
 *     summary: Listar emisiones por comunidad
 *     security:
 *       - bearerAuth: []
 *     parameters:
 *       - in: path
...
router.get('/comunidad/:comunidadId', authenticate, async (req, res) => {
  ...
  const [rows] = await db.query('SELECT id, periodo, estado, fecha_vencimiento FROM emision_gasto_comun WHERE comunidad_id = ? ORDER BY periodo DESC LIMIT ? OFFSET ?', [comunidadId, Number(limit), Number(offset)]);
  res.json(rows);
});
...
router.post('/comunidad/:comunidadId', [authenticate, authorize('admin','superadmin'), body('periodo').notEmpty(), body('fecha_vencimiento').notEmpty()], async (req, res) => {
  ...
  try { const [result] = await db.query('INSERT INTO emision_gasto_comun (comunidad_id, periodo, fecha_vencimiento, observaciones) VALUES (?,?,?,?)', [comunidadId, periodo, fecha_vencimiento, observaciones || null]); const [row] = await db.query('SELECT id, periodo, estado FROM emision_gasto_comun WHERE id = ? LIMIT 1', [result.insertId]); res.status(201).json(row[0]); } catch (err) { console.error(err); res.status(500).json({ error: 'server error' }); }
});

router.get('/:id', authenticate, async (req, res) => { const id = req.params.id; const [rows] = await db.query('SELECT * FROM emision_gasto_comun WHERE id = ? LIMIT 1', [id]); if (!rows.length) return res.status(404).json({ error: 'not found' }); res.json(rows[0]); });

router.patch('/:id', authenticate, authorize('admin','superadmin'), async (req, res) => { const id = req.params.id; if (!req.body.estado) return res.status(400).json({ error: 'estado required' }); try { await db.query('UPDATE emision_gasto_comun SET estado = ? WHERE id = ?', [req.body.estado, id]); const [rows] = await db.query('SELECT id, periodo, estado FROM emision_gasto_comun WHERE id = ? LIMIT 1',[id]); res.json(rows[0]); } catch (err) { console.error(err); res.status(500).json({ error: 'server error' }); } });
...
router.post('/:id/detalles', [authenticate, authorize('admin','superadmin'), body('categoria_id').isInt(), body('monto').isNumeric(), body('regla_prorrateo').notEmpty()], async (req, res) => {
  ...
  try { const [result] = await db.query('INSERT INTO emision_gasto_detalle (emision_id, gasto_id, categoria_id, monto, regla_prorrateo, metadata_json) VALUES (?,?,?,?,?,?)', [emisionId, gasto_id || null, categoria_id, monto, regla_prorrateo, metadata_json || null]); const [row] = await db.query('SELECT id, categoria_id, monto, regla_prorrateo FROM emision_gasto_detalle WHERE id = ? LIMIT 1', [result.insertId]); res.status(201).json(row[0]); } catch (err) { console.error(err); res.status(500).json({ error: 'server error' }); }
});
...
router.get('/:id/previsualizar-prorrateo', authenticate, async (req, res) => {
  ...
  try {
    ...
    const [emRows] = await db.query('SELECT comunidad_id, periodo FROM emision_gasto_comun WHERE id = ? LIMIT 1', [emisionId]);
    if (!emRows.length) return res.status(404).json({ error: 'emision not found' });
    const comunidadId = emRows[0].comunidad_id;
    const [detalles] = await db.query('SELECT id, gasto_id, categoria_id, monto, regla_prorrateo, metadata_json FROM emision_gasto_detalle WHERE emision_id = ?', [emisionId]);
...
router.post('/:id/generar-cargos', authenticate, authorize('admin','superadmin'), async (req, res) => {
  ...
  try {
    ...
    const [emRows] = await conn.query('SELECT comunidad_id FROM emision_gasto_comun WHERE id = ? LIMIT 1', [emisionId]);
    if (!emRows.length) { await conn.rollback(); return res.status(404).json({ error: 'emision not found' }); }
    const comunidadId = emRows[0].comunidad_id;

    const [detalles] = await conn.query('SELECT id, gasto_id, categoria_id, monto, regla_prorrateo, metadata_json FROM emision_gasto_detalle WHERE emision_id = ?', [emisionId]);
    ...
    // create cargo_unidad rows and cargo_unidad_detalle
    const created = [];
    for (const d of distrib) {
      const [r] = await conn.query('INSERT INTO cargo_unidad (emision_id, comunidad_id, unidad_id, monto_total, saldo) VALUES (?,?,?,?,?)', [emisionId, comunidadId, d.unidad_id, d.total, d.total]);
      const cargoId = r.insertId;
      for (const det of d.detalles) {
        await conn.query('INSERT INTO cargo_unidad_detalle (cargo_unidad_id, categoria_id, glosa, monto, origen, origen_id, iva_incluido) VALUES (?,?,?,?,?,?,?)', [cargoId, det.categoria_id, `detalle ${det.detalle_id}`, det.monto, 'gasto', det.gasto_id || null, 1]);
      }
      created.push({ unidad_id: d.unidad_id, cargo_unidad_id: cargoId, monto_total: d.total });
    }
