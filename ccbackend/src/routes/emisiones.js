 ...
 * summary: Obtener conceptos/prorrateo de una emisión
 * description: Obtiene todos los conceptos y reglas de prorrateo asociados a una emisión específica.
 * // ... (Resto de OpenAPI docs)
 */
router.get('/:id/conceptos', authenticate, async (req, res) => {
  const emisionId = req.params.id;
  try {
    // Usando consulta 4 de CONSULTAS_EMISIONES.sql (adaptada)
    const [rows] = await db.query(`
      SELECT
        deg.id,
        cg.nombre as nombre,
...
router.get('/:id/gastos', authenticate, async (req, res) => {
  const emisionId = req.params.id;
  try {
    // Usando consulta 5 de CONSULTAS_EMISIONES.sql (adaptada)
    const [rows] = await db.query(`
      SELECT
        g.id,
        g.glosa as descripcion,
...
router.get('/:id/unidades', authenticate, async (req, res) => {
  const emisionId = req.params.id;
  try {
    // Usando consulta 6 de CONSULTAS_EMISIONES.sql (adaptada, con corrección de titularidad)
    const [rows] = await db.query(`
      SELECT
        u.id,
        u.codigo as numero,
      ...
      LEFT JOIN (
          ...
          WHERE tu1.tipo IN ('propietario', 'arrendatario') -- Buscar activo
            AND (tu1.hasta IS NULL OR tu1.hasta >= CURDATE())
            AND NOT EXISTS (
                SELECT 1 FROM titulares_unidad tu2
                WHERE tu2.unidad_id = tu1.unidad_id
                  AND (tu2.hasta IS NULL OR tu2.hasta >= CURDATE())
                  AND tu2.created_at > tu1.created_at -- Más reciente
            )
...
router.get('/:id/pagos', authenticate, async (req, res) => {
  const emisionId = req.params.id;
  try {
    // Usando consulta 7 de CONSULTAS_EMISIONES.sql
    const [rows] = await db.query(`
      SELECT
        p.id,
        DATE_FORMAT(p.fecha, '%Y-%m-%d') as fecha,
...
router.get('/:id/historial', authenticate, async (req, res) => {
  const emisionId = req.params.id;
  try {
    // Usando consulta 8 de CONSULTAS_EMISIONES.sql (adaptada)
    const [rows] = await db.query(`
      SELECT
        a.id,
        DATE_FORMAT(a.created_at, '%Y-%m-%d %H:%i:%s') as fecha,
