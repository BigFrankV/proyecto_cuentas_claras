...
class SchedulerService {
      ...
      cron.schedule(
        ...
        async () => {
          ...
          try {
            ...
            if (!hasDataToday) {
              logger.info(
                'No hay datos para hoy, ejecutando sincronización...'
              );
              await indicadoresService.sincronizar();
            } else {
              logger.info(
                'Ya hay datos para hoy, omitiendo sincronización de respaldo'
              );
            }
          } catch (error) {
      ...
      cron.schedule(
        ...
        async () => {
          try {
            ...
            logger.info(
              `Estadísticas de datos: UF: ${stats.uf_count} registros, UTM: ${stats.utm_count} registros`
            );
          } catch (error) {
  ...
  async getDataStats() {
    ...
    try {
      const [ufCount] = await db.query(
        'SELECT COUNT(*) as count FROM uf_valor'
      );
      const [utmCount] = await db.query(
        'SELECT COUNT(*) as count FROM utm_valor'
      );
      const [lastUF] = await db.query(
        'SELECT fecha, valor FROM uf_valor ORDER BY fecha DESC LIMIT 1'
      );
      const [lastUTM] = await db.query(
        'SELECT fecha, valor FROM utm_valor ORDER BY fecha DESC LIMIT 1'
      );
  ...
  async cleanupOldData() {
    ...
    try {
      ...
      logger.info(
        `Limpieza completada: UF: ${ufResult.affectedRows}, UTM: ${utmResult.affectedRows}, Otros: ${otrosResult.affectedRows} registros eliminados`
      );
