...
function requireCommunity(paramName = 'comunidadId', allowedRoles = []) {
  return async (req, res, next) => {
    try {
      ...
      const tokenRoles = (req.user.roles || []).map((r) =>
      ...
      if (!comunidadId)
        return res.status(400).json({ error: 'missing comunidad id' });
      ...
      if (
        (allowedRoles || []).some((r) =>
          tokenRoles.includes(String(r).toLowerCase())
        )
      )
        return next();
      ...
      console.error('Error en requireCommunity:', err);
...
function requireUnidad(paramName = 'id') {
  return async (req, res, next) => {
    try {
      ...
      if (!unidadId)
        return res.status(400).json({ error: 'invalid unidad id' });
      ...
      if (usuarioId) {
        const [mrows] = await db.query(
          'SELECT 1 FROM usuario_rol_comunidad WHERE usuario_id = ? AND comunidad_id = ? AND activo = 1 LIMIT 1',
      ...
      console.error('Error en requireUnidad:', err);
      return res.status(500).json({ error: 'server error' });
    }
  };
}
