...
-- ============================================
-- MIGRACIÓN 1: AGREGAR COLUMNA slug A rol_sistema
-- ============================================
...
-- 2. Copiar valores de codigo a slug
UPDATE `rol_sistema` 
SET `slug` = `codigo`;

-- 3. Hacer slug obligatorio y único
ALTER TABLE `rol_sistema` 
MODIFY COLUMN `slug` VARCHAR(100) NOT NULL,
ADD UNIQUE KEY `idx_slug` (`slug`);

-- Verificar
SELECT id, codigo, slug, nombre FROM rol_sistema;

-- ============================================
-- MIGRACIÓN 2: CONVERTIR usuario_miembro_comunidad A VISTA
-- ============================================

-- 1. Renombrar tabla existente a temporal (por seguridad)
RENAME TABLE `usuario_miembro_comunidad` TO `usuario_miembro_comunidad_old`;

-- 2. Crear vista usuario_miembro_comunidad
...
-- 4. Si todo está OK, eliminar la tabla temporal
-- DROP TABLE IF EXISTS `usuario_miembro_comunidad_old`;
...
SELECT 'Tabla rol_sistema' AS verificacion, 
  CASE WHEN COUNT(*) > 0 THEN 'OK' ELSE 'ERROR' END AS resultado
FROM information_schema.columns 
WHERE table_schema = DATABASE() 
  AND table_name = 'rol_sistema' 
  AND column_name = 'slug'

UNION ALL

SELECT 'Vista usuario_miembro_comunidad', 
...
SELECT 'Tabla gasto - columna numero', 
  CASE WHEN COUNT(*) > 0 THEN 'OK' ELSE 'ERROR' END
FROM information_schema.columns 
WHERE table_schema = DATABASE() 
...
SELECT 'Tabla gasto - columna estado', 
  CASE WHEN COUNT(*) > 0 THEN 'OK' ELSE 'ERROR' END
FROM information_schema.columns 
WHERE table_schema = DATABASE() 
  AND table_name = 'gasto' 
...
SELECT 'Tabla historial_gasto', 
  CASE WHEN COUNT(*) > 0 THEN 'OK' ELSE 'ERROR' END
FROM information_schema.tables 
WHERE table_schema = DATABASE() 
  AND table_name = 'historial_gasto';

-- ============================================
-- FIN DE MIGRACIONES
-- ============================================

🔧 MIGRACIONES COMPLETAS PARA EL SISTEMA DE MULTAS
-- ============================================
USE ccdb;

-- ============================================
-- MIGRACIÓN 1: NUEVAS COLUMNAS EN TABLA multa
-- ============================================
...
-- 2. Agregar columna estado
ALTER TABLE `multa` 
ADD COLUMN `estado` ENUM('pendiente','pagada','anulada','condonada') NOT NULL DEFAULT 'pendiente' 
AFTER `fecha_limite`;

-- 3. Agregar columna creada_por
ALTER TABLE `multa` 
ADD COLUMN `creada_por` BIGINT DEFAULT NULL 
AFTER `estado`;

-- 4. Agregar columna aprobada_por
ALTER TABLE `multa` 
ADD COLUMN `aprobada_por` BIGINT DEFAULT NULL 
AFTER `creada_por`;

-- 5. Agregar columna fecha_aprobacion
ALTER TABLE `multa` 
ADD COLUMN `fecha_aprobacion` DATETIME DEFAULT NULL 
AFTER `aprobada_por`;

-- 6. Agregar índices
ALTER TABLE `multa` 
ADD INDEX `idx_multa_estado` (`estado`),
ADD INDEX `idx_multa_numero` (`numero`),
ADD INDEX `idx_multa_fecha_aprobacion` (`fecha_aprobacion`);

-- 7. Agregar foreign keys
ALTER TABLE `multa` 
ADD CONSTRAINT `fk_multa_creada_por` 
FOREIGN KEY (`creada_por`) REFERENCES `usuario` (`id`);

ALTER TABLE `multa` 
ADD CONSTRAINT `fk_multa_aprobada_por` 
FOREIGN KEY (`aprobada_por`) REFERENCES `usuario` (`id`);

-- 8. Generar números para multas existentes
UPDATE `multa` 
SET `numero` = CONCAT('M', YEAR(fecha_emision), '-', LPAD(id, 6, '0'))
WHERE `numero` IS NULL OR `numero` = '';

-- Verificar
SELECT id, numero, estado, monto, fecha_emision, creada_por, aprobada_por 
FROM multa 
LIMIT 5;

-- ============================================
-- MIGRACIÓN 2: TABLA historial_multa
-- ============================================

CREATE TABLE IF NOT EXISTS `historial_multa` (
  `id` BIGINT NOT NULL AUTO_INCREMENT,
  `multa_id` BIGINT NOT NULL COMMENT 'ID de la multa',
  `accion` ENUM('creada','modificada','aprobada','rechazada','pagada','anulada','condonada') NOT NULL COMMENT 'Acción realizada',
  `usuario_id` BIGINT NOT NULL COMMENT 'Usuario que realizó la acción',
  `monto_anterior` DECIMAL(15,2) DEFAULT NULL COMMENT 'Monto antes del cambio',
  `monto_nuevo` DECIMAL(15,2) DEFAULT NULL COMMENT 'Monto después del cambio',
  `observaciones` TEXT COMMENT 'Observaciones o motivo del cambio',
  `fecha` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'Fecha y hora de la acción',
  PRIMARY KEY (`id`),
  KEY `fk_historial_multa_multa` (`multa_id`),
  KEY `fk_historial_multa_usuario` (`usuario_id`),
  KEY `idx_historial_multa_fecha` (`fecha`),
  KEY `idx_historial_multa_accion` (`accion`),
  CONSTRAINT `fk_historial_multa_multa` 
    FOREIGN KEY (`multa_id`) REFERENCES `multa` (`id`) ON DELETE CASCADE,
  CONSTRAINT `fk_historial_multa_usuario` 
    FOREIGN KEY (`usuario_id`) REFERENCES `usuario` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
COMMENT='Historial de cambios y acciones sobre multas';

-- Índice compuesto para consultas frecuentes
CREATE INDEX `idx_historial_multa_id_fecha` ON `historial_multa` (`multa_id`, `fecha` DESC);

-- Verificar estructura
DESCRIBE historial_multa;
...
SELECT 
  '1. Tabla multa - columna numero' AS verificacion,
  CASE WHEN COUNT(*) > 0 THEN '✅ OK' ELSE '❌ ERROR' END AS resultado
FROM information_schema.columns 
WHERE table_schema = DATABASE() 
...
SELECT 
  '3. Tabla historial_multa',
  CASE WHEN COUNT(*) > 0 THEN '✅ OK' ELSE '❌ ERROR' END
FROM information_schema.tables 
WHERE table_schema = DATABASE() 
  AND table_name = 'historial_multa'

UNION ALL

SELECT 
  '4. Tabla apelacion_multa',
  CASE WHEN COUNT(*) > 0 THEN '✅ OK' ELSE '❌ ERROR' END
FROM information_schema.tables 
WHERE table_schema = DATABASE() 
  AND table_name = 'apelacion_multa'

UNION ALL

SELECT 
  '5. Tabla documento_multa',
  CASE WHEN COUNT(*) > 0 THEN '✅ OK' ELSE '❌ ERROR' END
FROM information_schema.tables 
WHERE table_schema = DATABASE() 
  AND table_name = 'documento_multa'

UNION ALL

SELECT 
  '6. Trigger creación multa',
  CASE WHEN COUNT(*) > 0 THEN '✅ OK' ELSE '❌ ERROR' END
FROM information_schema.triggers 
WHERE trigger_schema = DATABASE() 
  AND trigger_name = 'trg_multa_after_insert'

UNION ALL

SELECT 
  '7. Trigger modificación multa',
  CASE WHEN COUNT(*) > 0 THEN '✅ OK' ELSE '❌ ERROR' END
FROM information_schema.triggers 
WHERE trigger_schema = DATABASE() 
  AND trigger_name = 'trg_multa_after_update';

-- Ver resumen de datos
SELECT 
  'Multas totales' AS tabla, COUNT(*) AS cantidad FROM multa
UNION ALL
SELECT 'Historial multas', COUNT(*) FROM historial_multa
UNION ALL
SELECT 'Apelaciones', COUNT(*) FROM apelacion_multa
UNION ALL
SELECT 'Documentos', COUNT(*) FROM documento_multa;

-- ============================================
-- FIN DE MIGRACIONES DE MULTAS
-- ============================================
