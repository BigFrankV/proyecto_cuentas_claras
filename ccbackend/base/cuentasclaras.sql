...
-- 1. ✅ AGREGAR COLUMNA ACTIVO A PROVEEDOR
ALTER TABLE `proveedor` 
ADD COLUMN `activo` tinyint(1) NOT NULL DEFAULT '1' AFTER `direccion`;

-- 2. ✅ TABLA DE SESIONES DE USUARIO (CRÍTICA)
CREATE TABLE `sesion_usuario` (
  ...
  KEY `ix_sesion_usuario_last_activity` (`last_activity`),
  KEY `ix_sesion_usuario_created` (`created_at`),
  CONSTRAINT `fk_sesion_usuario` FOREIGN KEY (`usuario_id`) REFERENCES `usuario` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

-- 3. ✅ TABLA DE AUDITORÍA (NECESARIA)
CREATE TABLE `auditoria` (
  ...
  KEY `ix_auditoria_usuario_fecha` (`usuario_id`, `created_at`),
  CONSTRAINT `fk_audit_usuario` FOREIGN KEY (`usuario_id`) REFERENCES `usuario` (`id`) ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

-- 4. ✅ MEJORAS EN TABLA GASTO
ALTER TABLE gasto ADD COLUMN IF NOT EXISTS `numero` varchar(50) DEFAULT NULL AFTER `id`;
ALTER TABLE gasto ADD COLUMN IF NOT EXISTS `estado` enum('borrador','aprobado','rechazado','pagado','anulado') NOT NULL DEFAULT 'aprobado' AFTER `extraordinario`;
ALTER TABLE gasto ADD COLUMN IF NOT EXISTS `creado_por` bigint DEFAULT NULL AFTER `estado`;
ALTER TABLE gasto ADD COLUMN IF NOT EXISTS `aprobado_por` bigint DEFAULT NULL AFTER `creado_por`;

-- Índices para gasto
ALTER TABLE gasto ADD INDEX IF NOT EXISTS idx_gasto_estado (estado);
ALTER TABLE gasto ADD INDEX IF NOT EXISTS idx_gasto_numero (numero);

-- Foreign keys para gasto
ALTER TABLE gasto ADD CONSTRAINT IF NOT EXISTS fk_gasto_creado_por 
  FOREIGN KEY (creado_por) REFERENCES persona (id);
ALTER TABLE gasto ADD CONSTRAINT IF NOT EXISTS fk_gasto_aprobado_por 
  FOREIGN KEY (aprobado_por) REFERENCES persona (id);

-- 5. ✅ TABLA HISTORIAL GASTO
...
-- 6. ✅ DATOS DE EJEMPLO PARA AUDITORÍA
...
-- 7. ✅ ACTUALIZAR GASTOS EXISTENTES (OPCIONAL)
UPDATE gasto SET 
