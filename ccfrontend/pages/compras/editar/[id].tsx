...
import { Button, Card, Form, Row, Col, Alert, Table, Modal, Badge } from 'react-bootstrap';
...
export default function EditarCompra() {
  ...
  const loadPurchaseData = async () => {
    try {
      ...
      const mockPurchase: Purchase = {
        ...
        requestJustification: 'Se requiere reparar filtraciones en el techo del edificio A'
      };
  ...
  const loadInitialData = async () => {
    try {
      ...
      const mockProviders: Provider[] = [
        { id: 1, name: 'Materiales San Fernando Ltda.', category: 'construction', rating: 4.5, status: 'active' },
        { id: 2, name: 'Limpiezas Profesionales Vitacura Ltda.', category: 'services', rating: 4.8, status: 'active' },
        { id: 3, name: 'Ferretería Industrial Maipú', category: 'supplies', rating: 4.2, status: 'active' },
        { id: 4, name: 'Servicios Eléctricos Las Condes SpA', category: 'services', rating: 4.6, status: 'active' },
        { id: 5, name: 'Vivero y Paisajismo Lo Barnechea Ltda.', category: 'services', rating: 4.3, status: 'active' }
      ];

      const mockCostCenters: CostCenter[] = [
        { id: 1, name: 'Mantenimiento General', department: 'maintenance', budget: 8500000, spent: 2850000 },
        { id: 2, name: 'Servicios Generales', department: 'operations', budget: 6200000, spent: 1850000 },
        { id: 3, name: 'Administración', department: 'administration', budget: 4500000, spent: 1200000 },
        { id: 4, name: 'Jardinería y Paisajismo', department: 'operations', budget: 3800000, spent: 950000 },
        { id: 5, name: 'Seguridad', department: 'security', budget: 7500000, spent: 2800000 }
      ];
  ...
  useEffect(() => {
    if (originalData) {
      ...
      const itemsChanged = JSON.stringify(items) !== JSON.stringify(originalData.items);
  ...
  const savePurchase = async (requiresReapproval = false) => {
    ...
    try {
      ...
      if (requiresReapproval) {
        alert('Compra actualizada. Se ha enviado para re-aprobación debido a los cambios realizados.');
      } else {
  ...
  const canEdit = () => {
    return purchase?.status === 'draft' || purchase?.status === 'pending' || purchase?.status === 'approved';
  };
      ...
      <ProtectedRoute>
        <Layout>
          <div className="text-center py-5">
            ...
            <p className="text-muted">La compra que intentas editar no existe o ha sido eliminada.</p>
  ...
  if (isReadOnly()) {
    return (
      <ProtectedRoute>
        <Layout>
          <div className="text-center py-5">
            <span className="material-icons text-warning mb-3" style={{ fontSize: '4rem' }}>
              lock
            </span>
            <h4>Compra no editable</h4>
            <p className="text-muted">
              Esta compra está en estado "{getStatusInfo(purchase.status).label}" y no puede ser editada.
            </p>
    ...
    <ProtectedRoute>
      ...
      <Layout>
        <div className="purchases-container">
          ...
          {purchase.status === 'approved' && (
            <Alert variant="warning" className="mb-4">
              ...
              <strong>Atención:</strong> Esta compra ya está aprobada. 
              Los cambios significativos requerirán re-aprobación y volverán el estado a "Pendiente".
            </Alert>
          )}
                ...
                <Card className="purchase-form-section mb-4">
                  ...
                  <Card.Body>
                    <Row className="g-3">
                      <Col md={6}>
                        <Form.Group>
                          <Form.Label className="required">Tipo de Compra</Form.Label>
                          <Form.Select
                            value={formData.type}
                            onChange={(e) => handleInputChange('type', e.target.value)}
                          >
                            <option value="order">Orden de Compra</option>
                            <option value="service">Contratación de Servicio</option>
                      ...
                      <Col md={6}>
                        <Form.Group>
                          <Form.Label className="required">Prioridad</Form.Label>
                          <Form.Select
                      ...
                      <Col md={6}>
                        <Form.Group>
                          <Form.Label className="required">Fecha Requerida</Form.Label>
                          <Form.Control
                            type="date"
                          ...
                          >
                            ...
                            {providers.map(provider => (
                              <option key={provider.id} value={provider.id}>
                                {provider.name} - {provider.category} (⭐ {provider.rating})
                              </option>
                            ))}
                          </Form.Select>
                          <Form.Control.Feedback type="invalid">
                ...
                <Card className="purchase-form-section mb-4">
                  ...
                  <Card.Body>
                    <Row className="g-3">
                      <Col md={6}>
                        <Form.Group>
                          <Form.Label className="required">Centro de Costo</Form.Label>
                          <Form.Select
                          ...
                          >
                            ...
                            {costCenters.map(center => (
                              <option key={center.id} value={center.id}>
                                ...
                                (Disponible: {formatCurrency(center.budget - center.spent, formData.currency)})
                              </option>
                            ))}
                          </Form.Select>
                          <Form.Control.Feedback type="invalid">
                    ...
                    {showBudgetAlert && (
                      <Alert variant="warning" className="mt-3">
                        <span className="material-icons me-2">warning</span>
                        El monto total excede el presupuesto disponible del centro de costo seleccionado.
                      </Alert>
                    )}
                  </Card.Body>
                </Card>
                      ...
                      <Col xs={12}>
                        <Form.Group>
                          ...
                          <Form.Control
                            ...
                            onChange={(e) => handleInputChange('requestJustification', e.target.value)}
                          />
                        </Form.Group>
                      </Col>
                    </Row>
                ...
                <Card className="purchase-form-section mb-4">
                  ...
                  <Card.Body>
                    ...
                    <div className="small text-muted">
                      <div><strong>Solicitada por:</strong> {purchase.requestedBy}</div>
                      <div><strong>Fecha de solicitud:</strong> {new Date(purchase.requestedDate).toLocaleDateString('es-ES')}</div>
                      {purchase.status === 'approved' && (
                ...
                <Card className="purchase-form-section">
                  ...
                  <Card.Body>
                    <div className="info-list">
                      ...
                      <div className="info-item">
                        <span className="material-icons text-warning">refresh</span>
                        <div>
                          <strong>Re-aprobación</strong>
                          <p>Cambios en compras aprobadas requieren nueva aprobación.</p>
                        </div>
                      </div>    
                    </div>
                  </Card.Body>
        ...
        <Modal show={showConfirmModal} onHide={() => setShowConfirmModal(false)} centered>
          ...
          <Modal.Body>
            <p>Esta compra está <strong>aprobada</strong>. Los cambios que realizaste requieren una nueva aprobación.</p>
            <p>¿Deseas continuar? El estado cambiará a "Pendiente" y deberá ser aprobada nuevamente.</p>
          </Modal.Body>
          <Modal.Footer>
