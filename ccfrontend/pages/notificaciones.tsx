...
export default function NotificacionesListado() {
      ...
      const mockNotifications: NotificationData[] = [
        {
          ...
          message: 'Se realizará mantenimiento preventivo de los ascensores el próximo sábado de 8:00 AM a 12:00 PM. Durante este período, los ascensores no estarán disponibles.',
          type: 'maintenance',
          status: 'sent',
        ...
        },
        {
          ...
          message: 'Se convoca a todos los propietarios a la reunión ordinaria que se realizará el 25 de enero a las 19:00 horas en el salón de eventos.',
          type: 'announcement',
          status: 'scheduled',
          channels: ['email', 'app'],
          audience: {
        ...
        },
        {
          ...
          message: 'Te recordamos que el vencimiento para el pago de gastos comunes de enero es el 31 de enero. Puedes pagar a través de la app o en nuestras oficinas.',
          type: 'reminder',
          status: 'draft',
          channels: ['email', 'sms'],
          audience: {
        ...
        },
        {
          ...
          message: 'Informamos que habrá un corte de agua el día miércoles de 10:00 AM a 2:00 PM por trabajos de mantención en la red.',
          type: 'alert',
          status: 'failed',
  ...
  useEffect(() => {
    ...
    if (filters.channel) {
      filtered = filtered.filter(notification => 
        notification.channels.includes(filters.channel as 'email' | 'sms' | 'push' | 'app')
      );
    }
  ...
  const handleNotificationAction = async (action: string, notificationId: string) => {
    switch (action) {
      ...
      case 'duplicate':
        ...
        const notificationToDuplicate = notifications.find(n => n.id === notificationId);
        ...
        break;
      case 'resend':
        if (confirm('¿Estás seguro de que quieres reenviar esta notificación?')) {
          try {
            // Simular API call
  ...
  const handleBulkAction = async (action: string) => {
    ...
    switch (action) {
      case 'resend':
        if (confirm(`¿Estás seguro de que quieres reenviar ${selectedNotifications.length} notificación(es)?`)) {
          try {
            // Simular API call
            await new Promise(resolve => setTimeout(resolve, 1500));
            alert(`${selectedNotifications.length} notificación(es) reenviada(s) exitosamente`);
            // Actualizar las notificaciones seleccionadas
            setNotifications(prev => 
              prev.map(n => selectedNotifications.includes(n.id) 
                ? { ...n, sentAt: new Date().toISOString(), status: 'sent' as const }
                : n
              )
            );
            setSelectedNotifications([]);
        ...
        break;
      case 'archive':
        if (confirm(`¿Estás seguro de que quieres archivar ${selectedNotifications.length} notificación(es)?`)) {
          alert('Funcionalidad de archivo no implementada aún');
          setSelectedNotifications([]);
        }
        break;
      case 'delete':
        if (confirm(`¿Estás seguro de que quieres eliminar ${selectedNotifications.length} notificación(es)? Esta acción no se puede deshacer.`)) {
          setNotifications(prev => prev.filter(n => !selectedNotifications.includes(n.id)));
          setSelectedNotifications([]);
          alert(`${selectedNotifications.length} notificación(es) eliminada(s)`);
        }
        break;
      case 'export':
        ...
        const csvContent = "data:text/csv;charset=utf-8," 
          ...
          + selectedNotificationData.map(n => 
        ...
        <div className='container-fluid py-3'>
          ...
          <div className='d-flex justify-content-between align-items-center mb-4'>
            <div>
              ...
              <p className='text-muted mb-0'>Gestión de comunicaciones masivas</p>
            </div>
          ...
          <div className='row mb-4'>
            ...
            <div className='col-6 col-md-3 mb-3'>
              <div className='stats-card' style={{
                ...
                <div className='d-flex align-items-center'>
                  <div className='stats-icon secondary me-3' style={{
                    width: '60px',
                    height: '60px',
                    borderRadius: '50%',
                    display: 'flex',
            ...
            <div className='row g-3'>
              ...
              <div className='col-md-3 d-flex align-items-end gap-2'>
                <button
                  ...
                  onClick={() => {
                    setSearchTerm('');
                    setFilters({ status: '', type: '', channel: '', dateRange: '' });
                  }}
                >
                  Limpiar
                </button>
          ...
          {selectedNotifications.length > 0 && (
            <div className='bulk-actions mb-3' style={{
              ...
              <div className='d-flex justify-content-between align-items-center'>
                <span>{selectedNotifications.length} notificación(es) seleccionada(s)</span>
          ...
          <div className='notification-table' style={{
            ...
            <div className='notification-header' style={{
              ...
              <div className='d-flex justify-content-between align-items-center'>
                <h6 className='notification-title mb-0'>Notificaciones ({filteredNotifications.length})</h6>
                <div className='notification-actions d-flex gap-2'>
                  <button 
                    ...
                    onClick={() => {
                      ...
                      const link = document.createElement("a");
                      link.setAttribute("href", encodedUri);
                      link.setAttribute("download", `todas_notificaciones_${new Date().toISOString().split('T')[0]}.csv`);
                      document.body.appendChild(link);
                      link.click();
                      document.body.removeChild(link);
                  ...
                  <tbody>
                    {filteredNotifications.map((notification) => (
                      <tr key={notification.id}>
                        ...
                        <td>
                          <div className='notification-subject fw-semibold'>{notification.subject}</div>
                        ...
                        <td>
                          <span
                            ...
                            style={{
                              backgroundColor: statusConfig[notification.status].bg,
                        ...
                        <td>
                          <div className='notification-channels d-flex flex-wrap gap-1'>
                            {notification.channels.map((channel) => (
                              ...
                              >
                                <i className='material-icons me-1' style={{ fontSize: '12px' }}>
                        ...
                        <td>
                          ...
                          <div className='small text-muted'>{notification.audience.description}</div>
                        </td>
                        <td>
                          <div className='d-flex align-items-center'>
                            <div
                            ...
                            <span className='small'>{notification.author.name}</span>
                          </div>
                        </td>
                        <td>
                          <div className='small'>
            ...
            {viewMode === 'cards' || true ? (
              <div className={`p-3 ${viewMode === 'table' ? 'd-md-none' : ''}`}>
                <div className='row'>
                  {filteredNotifications.map((notification) => (
                    <div key={notification.id} className='col-12 col-md-6 col-lg-4 mb-3'>
                      <div 
                      ...
                      >
                        ...
                        <h6 className='notification-subject mb-2'>{notification.subject}</h6>
                        ...
                        <div className='notification-meta mb-3'>
                          <div className='notification-meta-item d-flex align-items-center mb-1'>
                            <i className='material-icons me-1' style={{ fontSize: '16px', color: '#6c757d' }}>people</i>
                            <span className='small text-muted'>{notification.audience.count} personas</span>
                          </div>
                          <div className='notification-meta-item d-flex align-items-center mb-1'>
