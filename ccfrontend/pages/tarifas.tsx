...
export default function TarifasListado() {
  ...
  // Estados para datos dinámicos - ANTES del useMemo
  ...
  // Determinar comunidadId según filtro global
  const resolvedComunidadId = React.useMemo(() => {
    if (isSuper) {
      return selectedComunidad?.id ? Number(selectedComunidad.id) : undefined;
    }
    return comunidadSeleccionada?.id ? Number(comunidadSeleccionada.id) : undefined;
  }, [isSuper, selectedComunidad, comunidadSeleccionada]);

  // Verificar si el usuario tiene rol básico
  ...
  // Cargar tarifas dinámicamente con filtro global
  useEffect(() => {
    if (!user) {
      return undefined;
    }
    ...
    const loadTarifas = async () => {
      ...
      try {
        ...
        if (isSuper) {
          ...
          // SUPERADMIN: cargar TODO o filtrado por comunidad_id
          resp = await listAllTarifasConsumo(params);
        } else {
          // USUARIO REGULAR: usar comunidad del filtro global
          if (!resolvedComunidadId) {
        ...
        console.log('✅ Tarifas cargadas:', resp.data?.length || 0, 'tarifas');
  ...
  }, [user, isSuper, selectedComunidad, resolvedComunidadId, comunidadSeleccionada, filterEstado, filterServicio, search]);
          ...
          <div className='container-fluid p-4'>
            ...
            {isBasicRoleInCommunity && (
              <div className='alert alert-info mb-3'>
                ...
                Solo puedes ver las tarifas. No tienes permisos para crear, editar o eliminar.
              </div>
            )}
                      ...
                      <div key={uiTarifa.id} className={`${styles['tariff-card']} mb-3 ${uiTarifa.servicio}`}>
                        <div className={`${styles['tariff-header']} d-flex justify-content-between align-items-start`}>
                          ...
                          <div className='mt-2 d-flex gap-2'>
                            <button className='btn btn-sm btn-outline-primary' onClick={() => router.push(`/tarifas/${uiTarifa.id}`)}>Ver</button>
                            {!isBasicRoleInCommunity && (
                              <>
