...
const ConsultorUF: React.FC = () => {
  ...
  const [calculatorResult, setCalculatorResult] = useState<UfCalculatorResult | null>(null);
    ...
    try {
      ...
      if (apiResponse.success) {
        ...
        setConsultaResult({
          ...
          fechaFormateada: '',
          success: false,
          errorMessage: apiResponse.error || 'Error al consultar el valor de la UF'
        });
      }
  ...
  const calcularConversion = (type: 'toPesos' | 'toUf') => {
    ...
    setTimeout(() => {
      const valorUf = calculatorInputs.valorUfActual || consultaResult?.valor || 37250;
  ...
  const cargarHistorico = async (period: '7d' | '30d' | '90d' | '1y') => {
    ...
    try {
      ...
      const apiResponse = await indicadoresAPI.getUfHistorico(dateRange.start, dateRange.end);
      ...
      if (apiResponse.success) {
        ...
        const data: UfHistoryItem[] = apiResponse.data.map((item, index) => {
          ...
          const variacionPorcentaje = prevValue > 0 ? (variacionAbsoluta / prevValue) * 100 : 0;
                        ...
                        <div className="alert alert-success">
                          <div className="d-flex align-items-center">
                            ...
                            <div className="flex-grow-1">
                              ...
                              <p className="mb-1">
                                <strong>Fecha:</strong> {consultaResult.fechaFormateada}
                              </p>
                              <p className="mb-0">
                                <strong>Valor:</strong> 
              ...
              <div className="card">
                ...
                <div className="card-body">
                  <p className="text-muted small">
                    La Unidad de Fomento (UF) es una unidad de cuenta reajustable 
                    de acuerdo con la inflación, establecida en Chile.
                  </p>
                  ...
                  <div className="card-body">
                    <div className="row text-center">
                      ...
                      <div className="col-md-4">
                        ...
                        <div>
                          {formatPesos(calculatorResult.valorUfUsado)}
                        </div>
                    ...
                    <div className="btn-group">
                      {[
                        ...
                        <button
                          ...
                          onClick={() => cargarHistorico(period.key as '7d' | '30d' | '90d' | '1y')}
                        >
                          {period.label}
                        </button>
                      ))}
                ...
                <div className="card-body">
                  ...
                  <div className="mb-4" style={{ height: '400px' }}>
                    {chartData && createChartConfig() && (
                      <Line data={createChartConfig()!} options={chartOptions} />
                    )}
                  </div>
                  ...
                  {historialUf.length > 0 && (
                    <div className="table-responsive">
                      <table className="table table-striped">
                        ...
                        <tbody>
                          {historialUf.slice(-10).reverse().map((item, index) => (
                            <tr key={index}>
                              <td>
                                {new Date(item.fecha).toLocaleDateString('es-CL')}
                              </td>
                              <td>{formatPesos(item.valor)}</td>
                              <td className={item.variacion >= 0 ? 'text-success' : 'text-danger'}>
                                <i className={`material-icons me-1 ${item.variacion >= 0 ? 'text-success' : 'text-danger'}`}>
                                  {item.variacion >= 0 ? 'trending_up' : 'trending_down'}
                                </i>
                                {formatPesos(Math.abs(item.variacion))}
                              </td>
                              <td className={item.variacionPorcentaje >= 0 ? 'text-success' : 'text-danger'}>
