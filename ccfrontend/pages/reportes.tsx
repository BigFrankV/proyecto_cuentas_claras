...
export default function ReportesDashboard() {
  ...
  const loadReportData = async () => {
    ...
    try {
      // Cargar gastos
      const gastosResponse = await apiClient.get(
        `/gastos/comunidad/${selectedCommunity}`,
      );
      setGastos(gastosResponse.data);

      // Cargar pagos
      const pagosResponse = await apiClient.get(
        `/pagos/comunidad/${selectedCommunity}`,
      );
  ...
  const handleExportFinancial = () => {
    const communityName =
      communities.find(c => c.id === selectedCommunity)?.nombre || 'comunidad';
    const result = exportService.exportFinancialReport(
      reportData,
      communityName,
    );
    ...
    <ProtectedRoute>
      ...
      <Layout title='Reportes y Analítica'>
        <div className='container-fluid fade-in'>
          ...
          <div className='d-flex justify-content-between align-items-center mb-4'>
            <div>
              ...
              <p className='text-muted mb-0'>
                Monitoreo detallado y análisis de datos
              </p>
            </div>
          ...
          {exportMessage && (
            <div
              className={`alert ${exportMessage.type === 'success' ? 'alert-success' : 'alert-danger'} alert-dismissible fade show mb-4`}
            >
