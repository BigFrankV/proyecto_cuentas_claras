...
import { Button, Card, Row, Col, Badge, Form, Dropdown, ButtonGroup, Alert, Modal, Table } from 'react-bootstrap';
...
export default function ConsumosMedidor() {
  ...
  const loadData = async () => {
    try {
      ...
      const mockConsumptionData: ConsumptionData[] = generateConsumptionData(meterData.type, selectedPeriod);
      ...
      const mockAlerts: Alert[] = [
        {
          ...
          description: 'El consumo del último período superó en 25% el promedio histórico',
        ...
        },
        {
          ...
          description: 'Detectado consumo fuera de horario habitual durante 3 días consecutivos',
  ...
  const generateConsumptionData = (meterType: string, period: string): ConsumptionData[] => {
    ...
    if (period === 'year') {
      for (let i = 11; i >= 0; i--) {
        ...
        const baseConsumption = meterType === 'electric' ? 850 : 
                              meterType === 'water' ? 15 : 45;
        
        // Variación estacional para Chile
        const seasonalFactor = getSummerSeasonalFactor(date.getMonth());
        const consumption = Math.round(baseConsumption * seasonalFactor + (Math.random() - 0.5) * 100);
        const previous = i === 11 ? consumption : data[data.length - 1]?.consumption || consumption;
        
        const consumptionEntry: ConsumptionData = {
          period: date.toLocaleDateString('es-CL', { month: 'short', year: 'numeric' }),
          consumption,
          cost: consumption * (meterType === 'electric' ? 156.8 : meterType === 'water' ? 890.5 : 620.3), // Precios chilenos aproximados
          previous,
          ...
          ...(meterType === 'electric' && { offPeak: Math.round(consumption * 0.4) }),
          ...(meterType === 'water' && { temperature: Math.round(12 + Math.random() * 8) }),
          ...(meterType === 'water' && { pressure: Math.round(2.8 + Math.random() * 0.8) })
        };
      ...
      for (let i = daysInMonth - 1; i >= 0; i--) {
        ...
        const baseDaily = meterType === 'electric' ? 28 : meterType === 'water' ? 0.5 : 1.5;
        const consumption = Math.round(baseDaily + (Math.random() - 0.5) * 10);
        
        const consumptionEntry: ConsumptionData = {
          period: date.toLocaleDateString('es-CL', { day: 'numeric', month: 'short' }),
          consumption,
          cost: consumption * (meterType === 'electric' ? 156.8 : meterType === 'water' ? 890.5 : 620.3),
          previous: i === daysInMonth - 1 ? consumption : data[data.length - 1]?.consumption || consumption,
          variation: Math.round((Math.random() - 0.5) * 50),
          average: Math.round(baseDaily),
          peak: Math.round(consumption * 1.4),
          ...(meterType === 'electric' && { offPeak: Math.round(consumption * 0.3) })
        };
        data.push(consumptionEntry);
      }
    } else if (period === 'week') {
      for (let i = 6; i >= 0; i--) {
        ...
        const consumptionEntry: ConsumptionData = {
          period: date.toLocaleDateString('es-CL', { weekday: 'short', day: 'numeric' }),
          consumption,
          cost: consumption * (meterType === 'electric' ? 156.8 : meterType === 'water' ? 890.5 : 620.3),
          previous: i === 6 ? consumption : data[data.length - 1]?.consumption || consumption,
  ...
  const getAverageConsumption = () => {
    return consumptionData.length > 0 ? getTotalConsumption() / consumptionData.length : 0;
  };

  const getUnit = () => {
    if (!meter) return '';
    return meter.type === 'electric' ? 'kWh' : 
           meter.type === 'water' ? 'm³' : 'm³';
  };
    ...
    datasets: [
      {
        ...
        backgroundColor: consumptionData.slice(-12).map(d => 
          d.variation > 15 ? '#e74c3c' : 
          d.variation < -15 ? '#27ae60' : '#3498db'
        ),
        borderColor: '#2c3e50',
        borderWidth: 1
      }
    ...
    <ProtectedRoute>
      ...
      <Layout>
        <div className="medidores-container">
                ...
                <Row className="align-items-center">
                  <Col>
                    <h1 className="page-title">
                      <span className="material-icons me-3" style={{ fontSize: '2.5rem' }}>analytics</span>
                ...
                <div className="d-flex justify-content-end gap-2">
                  <ButtonGroup>
                    {[
                      ...
                      <Button
                        key={view.key}
                        variant={selectedView === view.key ? 'primary' : 'outline-primary'}
                        size="sm"
                        onClick={() => setSelectedView(view.key)}
                      >
                        <span className="material-icons me-1" style={{ fontSize: '16px' }}>{view.icon}</span>
                        {view.label}
                      </Button>
                    ))}
                  </ButtonGroup>
            ...
            <Row className="g-4 mb-4">
                ...
                <div className="stats-card primary">
                  <div className="stats-card-content">
                    <div className="stats-card-value">
                      {Math.round(getTotalConsumption()).toLocaleString('es-CL')}
                      <span className="stats-card-unit">{getUnit()}</span>
                    </div>
                    <div className="stats-card-label">Consumo Total</div>
                    <div className="stats-card-period">{selectedPeriod === 'year' ? 'Último año' : selectedPeriod === 'month' ? 'Último mes' : 'Última semana'}</div>
                  </div>
              ...
              <Col md={3}>
                <div className="stats-card info">
                  <div className="stats-card-content">
                    <div className="stats-card-value">
                      {Math.round(getAverageConsumption()).toLocaleString('es-CL')}
                      <span className="stats-card-unit">{getUnit()}</span>
                    </div>
              ...
              <Col md={3}>
                <div className={`stats-card ${getConsumptionTrend() === 'up' ? 'warning' : getConsumptionTrend() === 'down' ? 'success' : 'info'}`}>
                  <div className="stats-card-content">
                    <div className="stats-card-value">
                      <span className="material-icons me-2">
                        {getConsumptionTrend() === 'up' ? 'trending_up' : 
                         getConsumptionTrend() === 'down' ? 'trending_down' : 'trending_flat'}
                      </span>
                      {getConsumptionTrend() === 'up' ? 'Alza' : 
                       getConsumptionTrend() === 'down' ? 'Baja' : 'Estable'}
                    </div>
                ...
                <Col>
                  <Alert variant="warning" className="d-flex align-items-center">
                    ...
                    <div className="flex-grow-1">
                      <strong>Alertas Activas:</strong> {alerts.filter(alert => !alert.resolved).length} alertas requieren atención
                    </div>
                    <Button
                  ...
                  <Card className="chart-card">
                    <Card.Header className="d-flex justify-content-between align-items-center">
                      ...
                      <div className="chart-controls">
                        <ButtonGroup size="sm">
                          <Button variant="outline-primary" active>Línea</Button>
                  ...
                  <Card className="chart-card">
                    ...
                    <Card.Body>
                      <div style={{ height: '300px', position: 'relative' }}>
                        <Doughnut data={doughnutData} options={{ ...chartOptions, plugins: { ...chartOptions.plugins, legend: { position: 'bottom' } } }} />
                      </div>
                    </Card.Body>
                  </Card>
                </Col>
                          ...
                          {consumptionData.map((data, index) => (
                            <tr key={index}>
                              ...
                              <td className="text-end">
                                <strong>{data.consumption.toLocaleString('es-CL')}</strong>
                              </td>
                              <td className="text-end">{getCurrency(data.cost)}</td>
                              <td className="text-center">
                                <Badge bg={data.variation > 15 ? 'danger' : data.variation < -15 ? 'success' : 'secondary'}>
                                  {data.variation > 0 ? '+' : ''}{data.variation}%
                                </Badge>
                              </td>
                              <td className="text-center">
                                <Badge bg={data.consumption > data.average * 1.1 ? 'warning' : 'success'}>
                                  {data.consumption > data.average ? 'Superior' : 'Normal'}
                                </Badge>
                              </td>
                              {meter.type === 'electric' && (
                                <td className="text-end">
                              ...
                              {meter.type === 'water' && (
                                <td className="text-end">{data.temperature ?? '-'}°C</td>
                              )}
                              {meter.type === 'water' && (
                                <td className="text-end">{data.pressure ?? '-'} bar</td>
                              )}
                            </tr>
                          ))}
                        </tbody>
          ...
          <Modal show={showAnomalyDetails} onHide={() => setShowAnomalyDetails(false)} size="lg">
            ...
            <Modal.Body>
              {alerts.map((alert) => (
                <div key={alert.id} className={`alert alert-${alert.severity === 'high' ? 'danger' : alert.severity === 'medium' ? 'warning' : 'info'} d-flex align-items-start`}>
                  <span className="material-icons me-3">
                    {alert.type === 'high' ? 'trending_up' : 
                     alert.type === 'low' ? 'trending_down' : 
                     alert.type === 'anomaly' ? 'report_problem' : 
                     alert.type === 'leak' ? 'water_drop' : 'compress'}
                  </span>
                  <div className="flex-grow-1">
                    <h6 className="alert-heading mb-1">
                      {alert.title}
                      {alert.resolved && <Badge bg="success" className="ms-2">Resuelto</Badge>}
                    </h6>
            ...
            <Modal.Footer>
              <Button variant="outline-secondary" onClick={() => setShowAnomalyDetails(false)}>
                Cerrar
              </Button>
            </Modal.Footer>
          </Modal>
          ...
          <Modal show={showExportModal} onHide={() => setShowExportModal(false)} centered>
            ...
            <Modal.Body>
              ...
              <div className="d-grid gap-2">
                <Button variant="outline-primary" onClick={() => exportData('json')}>
                ...
                <Button variant="outline-info" onClick={() => exportData('pdf')}>
