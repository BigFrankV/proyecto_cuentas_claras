...
export default function NuevoMedidor() {
  ...
  const [formData, setFormData] = useState({
    ...
    // Mantenimiento
    maintenanceFrequency: 'semestral' as 'mensual' | 'trimestral' | 'semestral' | 'anual',
    serviceCompany: '',
    firstServiceDate: '',
  ...
  useEffect(() => {
    ...
    if (formData.type) {
      const prefix = formData.type === 'electric' ? 'ELC' : 
                    formData.type === 'water' ? 'AGU' : 'GAS';
  ...
  const loadInitialData = async () => {
    try {
      ...
      const mockCommunities: Community[] = [
        { id: 1, name: 'Condominio Las Condes', address: 'Av. Las Condes 12345' },
        { id: 2, name: 'Residencial Vitacura', address: 'Av. Vitacura 6789' },
        { id: 3, name: 'Edificio Lo Barnechea', address: 'Av. La Dehesa 3456' },
        { id: 4, name: 'Centro Comercial Maipú', address: 'Av. Pajaritos 8901' }
      ];
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.type) newErrors.type = 'El tipo de medidor es requerido';
    if (!formData.serialNumber) newErrors.serialNumber = 'El número de serie es requerido';
    if (!formData.brand) newErrors.brand = 'La marca es requerida';
    if (!formData.model) newErrors.model = 'El modelo es requerido';
    if (!formData.communityId) newErrors.communityId = 'La comunidad es requerida';
    if (!formData.buildingId) newErrors.buildingId = 'El edificio es requerido';
    ...
    if (!formData.precision) newErrors.precision = 'La precisión es requerida';
    if (!formData.certification) newErrors.certification = 'La certificación es requerida';
    if (!formData.installationDate) newErrors.installationDate = 'La fecha de instalación es requerida';
    if (!formData.technician) newErrors.technician = 'El técnico instalador es requerido';
    if (!formData.company) newErrors.company = 'La empresa instaladora es requerida';

    // Validaciones específicas por tipo
    if (formData.type === 'water' && !formData.maxPressure) {
      newErrors.maxPressure = 'La presión máxima es requerida para medidores de agua';
    }

    // Validaciones de formato
    if (formData.serialNumber && formData.serialNumber.length < 5) {
      newErrors.serialNumber = 'El número de serie debe tener al menos 5 caracteres';
    }

    if (formData.consumptionThreshold < 0) {
      newErrors.consumptionThreshold = 'El umbral de consumo no puede ser negativo';
    }
                ...
                <Row className="align-items-center">
                  <Col>
                    <h1 className="page-title">
                      <span className="material-icons me-3" style={{ fontSize: '2.5rem' }}>add_circle</span>
                      Crear Nuevo Medidor
                    </h1>
                    <p className="page-subtitle">
                      Registra un nuevo medidor en el sistema con toda su información técnica
                    </p>
                  </Col>
                  <Col xs="auto">
                    <Button 
                ...
                <Col lg={8}>
                  <div className="form-card">
                    ...
                    <Row className="g-3 mb-4">
                      <Col>
                        <Form.Label className="required">Tipo de Medidor</Form.Label>
                        <Row className="g-3">
                          {(['electric', 'water', 'gas'] as const).map((type) => (
                            <Col md={4} key={type}>
                              ...
                              >
                                <span className="material-icons">{getTypeIcon(type)}</span>
                                <div className="type-selection-title">
                                  {type === 'electric' ? 'Eléctrico' : 
                                   type === 'water' ? 'Agua' : 'Gas Natural'}
                                </div>
                                <div className="type-selection-description">
                                  {type === 'electric' ? 'Medición de consumo eléctrico' : 
                                   type === 'water' ? 'Medición de consumo de agua' : 
                    ...
                    <Row className="g-3">
                      <Col md={6}>
                        <Form.Group>
                          <Form.Label className="required">Código del Medidor</Form.Label>
                          <Form.Control
                            type="text"
                            value={formData.code}
                            ...
                            readOnly
                          />
                          <Form.Text>Se genera automáticamente según el tipo</Form.Text>
                        </Form.Group>
                      </Col>
                      <Col md={6}>
                        <Form.Group>
                          <Form.Label className="required">Número de Serie</Form.Label>
                          <Form.Control
                            type="text"
                          ...
                          />
                          {errors.serialNumber && <Form.Control.Feedback type="invalid">{errors.serialNumber}</Form.Control.Feedback>}
                        </Form.Group>
                      </Col>
                      <Col md={6}>
                        <Form.Group>
                          ...
                          >
                            ...
                            {formData.type === 'water' && (
                              <>
                                <option value="Elster Honeywell">Elster Honeywell</option>
                            ...
                            {formData.type === 'gas' && (
                              <>
                                <option value="Elster Instromet">Elster Instromet</option>
                                <option value="Itron">Itron</option>
                    ...
                    <Row className="g-3">
                      <Col md={6}>
                        <Form.Group>
                          ...
                          {errors.communityId && <Form.Control.Feedback type="invalid">{errors.communityId}</Form.Control.Feedback>}
                        </Form.Group>
                      </Col>
                      <Col md={6}>
                        <Form.Group>
                          ...
                          {errors.buildingId && <Form.Control.Feedback type="invalid">{errors.buildingId}</Form.Control.Feedback>}
                        </Form.Group>
                      </Col>
                      <Col md={6}>
                        <Form.Group>
                          ...
                          >
                            ...
                            {units.map((unit) => (
                              <option key={unit.id} value={unit.id}>
                                {unit.number} - Piso {unit.floor} ({unit.type === 'apartment' ? 'Apartamento' : unit.type === 'commercial' ? 'Comercial' : 'Estacionamiento'})
                              </option>
                            ))}
                          </Form.Select>
                          {errors.unitId && <Form.Control.Feedback type="invalid">{errors.unitId}</Form.Control.Feedback>}
                        </Form.Group>
                      </Col>
                      <Col md={6}>
                        <Form.Group>
                          <Form.Label className="required">Posición del Medidor</Form.Label>
                          <Form.Control
                            type="text"
                          ...
                          />
                          {errors.position && <Form.Control.Feedback type="invalid">{errors.position}</Form.Control.Feedback>}
                        </Form.Group>
                      </Col>
                      <Col md={12}>
                        <Form.Group>
                  ...
                  <div className="form-card">
                    ...
                    <Row className="g-3">
                      <Col md={6}>
                        <Form.Group>
                          <Form.Label className="required">
                            {formData.type === 'electric' ? 'Capacidad (A)' : 
                             formData.type === 'water' ? 'Diámetro (mm)' : 
                             'Capacidad (m³/h)'}
                          </Form.Label>
                          <Form.Control
                            type="text"
                            ...
                            placeholder={formData.type === 'electric' ? 'Ej: 100 A' : 
                                       formData.type === 'water' ? 'Ej: 15 mm' : 
                                       'Ej: 6 m³/h'}
                      ...
                      <Col md={6}>
                        <Form.Group>
                          <Form.Label className="required">Precisión</Form.Label>
                          <Form.Control
                            type="text"
                            value={formData.precision}
                            onChange={(e) => handleInputChange('precision', e.target.value)}
                            placeholder={formData.type === 'electric' ? 'Ej: Clase 1 (±1%)' : 
                                       formData.type === 'water' ? 'Ej: R160' : 
                                       'Ej: Clase 1.5'}
                            isInvalid={!!errors.precision}
                          />
                          {errors.precision && <Form.Control.Feedback type="invalid">{errors.precision}</Form.Control.Feedback>}
                        </Form.Group>
                      </Col>
                      <Col md={6}>
                        <Form.Group>
                          <Form.Label className="required">Certificación</Form.Label>
                          <Form.Select
                          ...
                          >
                            ...
                            {formData.type === 'electric' && (
                              <>
                                <option value="SEC Aprobado - IEC 62053-21">SEC Aprobado - IEC 62053-21</option>
                                <option value="SEC Homologado - IEC 62052-11">SEC Homologado - IEC 62052-11</option>
                                <option value="ANSI C12.20">ANSI C12.20</option>
                              </>
                            )}
                            {formData.type === 'water' && (
                              <>
                                <option value="SISS Certificado - ISO 4064">SISS Certificado - ISO 4064</option>
                                <option value="SISS Aprobado - OIML R49">SISS Aprobado - OIML R49</option>
                                <option value="NSF 61 Certified">NSF 61 Certified</option>
                              </>
                            )}
                            {formData.type === 'gas' && (
                              <>
                                <option value="CNE Aprobado - ISO 17808">CNE Aprobado - ISO 17808</option>
                                <option value="SEC Gas Natural - OIML R137">SEC Gas Natural - OIML R137</option>
                                <option value="EN 1359 Certified">EN 1359 Certified</option>
                              </>
                            )}
                          </Form.Select>
                          {errors.certification && <Form.Control.Feedback type="invalid">{errors.certification}</Form.Control.Feedback>}
                        </Form.Group>
                      </Col>
                      <Col md={6}>
                        <Form.Group>
                      ...
                      {formData.type === 'water' && (
                        <Col md={6}>
                          <Form.Group>
                            <Form.Label className="required">Presión Máxima (bar)</Form.Label>
                            <Form.Control
                              type="text"
                            ...
                            />
                            {errors.maxPressure && <Form.Control.Feedback type="invalid">{errors.maxPressure}</Form.Control.Feedback>}
                          </Form.Group>
                        </Col>
                      )}
                      <Col md={6}>
                ...
                <Col lg={4}>
                    ...
                    {formData.type && (
                      <div className="mb-3">
                        <div className="d-flex align-items-center mb-2">
                          <span className="material-icons me-2 text-primary-custom">{getTypeIcon(formData.type)}</span>
                          <strong>
                            {formData.type === 'electric' ? 'Medidor Eléctrico' : 
                             formData.type === 'water' ? 'Medidor de Agua' : 
                             'Medidor de Gas'}
                          </strong>
                        </div>
                        ...
                        {formData.code && (
                          <div className="mb-2">
                            <small className="text-muted">Código:</small> <code>{formData.code}</code>
                          </div>
                        )}
                        
                        {formData.brand && formData.model && (
                          <div className="mb-2">
                            <small className="text-muted">Equipo:</small> {formData.brand} {formData.model}
                          </div>
                        )}
                        
                        {getSelectedCommunity() && (
                          <div className="mb-2">
                            <small className="text-muted">Comunidad:</small> {getSelectedCommunity()?.name}
                          </div>
                        )}
                        
                        {getSelectedBuilding() && getSelectedUnit() && (
                          <div className="mb-2">
                            <small className="text-muted">Ubicación:</small> {getSelectedBuilding()?.name} - {getSelectedUnit()?.number}
                          </div>
                        )}
                        
                        {formData.capacity && (
                          <div className="mb-2">
                            <small className="text-muted">Capacidad:</small> {formData.capacity}
                          </div>
                        )}
                      </div>
                    )}
                  ...
                  <div className="form-card">
                    ...
                    <Form.Group className="mb-3">
                      <Form.Label className="required">Fecha de Instalación</Form.Label>
                      <Form.Control
                        type="date"
                    ...
                    <Form.Group className="mb-3">
                      <Form.Label className="required">Técnico Instalador</Form.Label>
                      <Form.Control
                        type="text"
                      ...
                      />
                      {errors.technician && <Form.Control.Feedback type="invalid">{errors.technician}</Form.Control.Feedback>}
                    </Form.Group>
                    
                    <Form.Group className="mb-3">
                      <Form.Label className="required">Empresa Instaladora</Form.Label>
                      <Form.Control
                        type="text"
                      ...
                      />
                      {errors.company && <Form.Control.Feedback type="invalid">{errors.company}</Form.Control.Feedback>}
                    </Form.Group>
                    ...
                    <Form.Group className="mb-3">
                      ...
                      <Form.Control
                        ...
                        onChange={(e) => handleInputChange('warrantyUntil', e.target.value)}
                      />
                    </Form.Group>
                  ...
                  <div className="form-card">
                    ...
                    <div className="mb-3">
                      <Form.Check
                        ...
                        onChange={(e) => handleInputChange('autoReading', e.target.checked)}
                      />
                    </div>
              ...
              <Row>
                <Col md={6}>
                  ...
                  <ul className="list-unstyled">
                    <li><strong>Tipo:</strong> {formData.type === 'electric' ? 'Eléctrico' : formData.type === 'water' ? 'Agua' : 'Gas'}</li>
                    <li><strong>Código:</strong> {formData.code}</li>
                    <li><strong>Serie:</strong> {formData.serialNumber}</li>
                ...
                <Col md={6}>
                  <h6>Ubicación</h6>
                  <ul className="list-unstyled">
                    <li><strong>Comunidad:</strong> {getSelectedCommunity()?.name}</li>
                    <li><strong>Edificio:</strong> {getSelectedBuilding()?.name}</li>
                    <li><strong>Unidad:</strong> {getSelectedUnit()?.number}</li>
                    <li><strong>Posición:</strong> {formData.position}</li>
                  </ul>
                </Col>
                <Col md={6}>
                  <h6>Especificaciones</h6>
                  <ul className="list-unstyled">
                    <li><strong>Capacidad:</strong> {formData.capacity}</li>
                    <li><strong>Precisión:</strong> {formData.precision}</li>
                    <li><strong>Certificación:</strong> {formData.certification}</li>
                  </ul>
                </Col>
                <Col md={6}>
                  <h6>Instalación</h6>
                  <ul className="list-unstyled">
                    <li><strong>Fecha:</strong> {formData.installationDate ? new Date(formData.installationDate).toLocaleDateString() : '-'}</li>
                    <li><strong>Técnico:</strong> {formData.technician}</li>
