...
export default function NuevoMedidor() {
  ...
  // Verificar permisos - roles básicos NO pueden crear medidores
  const isBasicRoleInCommunity = React.useMemo(() => {
    if (!comunidadSeleccionada?.id) {return false;}
    const comunidadId = typeof comunidadSeleccionada.id === 'string' 
      ? parseInt(comunidadSeleccionada.id, 10) 
  ...
  }, [comunidadSeleccionada, hasRoleInCommunity]);

  // Recargar cuando cambie la comunidad (para actualizar permisos)
  useEffect(() => {
    // El useMemo de isBasicRoleInCommunity ya se encarga de actualizar
    // cuando cambia comunidadSeleccionada
  ...
  const loadInitialData = async () => {
    try {
      ...
      const mappedCommunities = comunidadesData.map(c => ({ id: c.id, name: c.nombre, address: c.direccion }));
      setCommunities(mappedCommunities);
      
      // Si hay comunidad seleccionada del filtro global, pre-seleccionarla
      if (comunidadSeleccionada?.id && mappedCommunities.length > 0) {
        const comunidadId = typeof comunidadSeleccionada.id === 'string' 
          ? comunidadSeleccionada.id 
          : String(comunidadSeleccionada.id);
        if (mappedCommunities.some(c => String(c.id) === comunidadId)) {
          setFormData(prev => ({ ...prev, communityId: comunidadId }));
        }
      }
    } catch (error) {
