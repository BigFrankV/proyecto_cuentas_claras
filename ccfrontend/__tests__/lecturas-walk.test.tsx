...
jest.mock('next/head', () => ({ __esModule: true, default: ({ children }: any) => React.createElement(React.Fragment, null, children) }));
jest.mock('next/router', () => ({ useRouter: () => ({ push: jest.fn(), query: { id: '1' } }) }));
jest.mock('@/components/layout/Layout', () => ({ __esModule: true, default: ({ children }: any) => React.createElement('div', { 'data-testid': 'mock-layout' }, children) }));
jest.mock('@/lib/useAuth', () => ({ __esModule: true, ProtectedRoute: ({ children }: any) => React.createElement('div', { 'data-testid': 'protected' }, children) }));
...
function walkElement(element: any, path: string[] = [], findings: any[] = []) {
  if (!element || typeof element !== 'object') return findings;
  ...
  if (Array.isArray(children)) {
    children.forEach((ch, i) => walkElement(ch, path.concat([`[${i}]`]), findings));
  } else if (children) {
...
test('walk lecturas element tree to find object-typed element types', () => {
  ...
  if (findings.length > 0) {
    throw new Error('Found object-typed element types: ' + JSON.stringify(findings));
  }
});
