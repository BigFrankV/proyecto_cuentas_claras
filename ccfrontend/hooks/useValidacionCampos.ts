...
export const useValidacionCampos = () => {
  ...
  const validarRut = useCallback(
    async (rut: string, excludeId?: number): Promise<ValidacionResultado> => {
      if (!rut || rut.trim() === '') {
  ...
  const validarEmail = useCallback(
    async (email: string, excludeId?: number): Promise<ValidacionResultado> => {
      if (!email || email.trim() === '') {
