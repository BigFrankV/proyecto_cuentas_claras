...
export const useRutValidator = () => {
  ...
  const calculateDigitoVerificador = useCallback((rutNumbers: string): string => {
    let suma = 0;
    let multiplicador = 2;
  ...
  const formatRut = useCallback((rut: string, format: 'dots' | 'clean' | 'dash' = 'dots'): string => {
  ...
  const validateRut = useCallback((rut: string): RutValidation => {
    const cleaned = cleanRut(rut);
    ...
    const result: RutValidation = {
      ...
      cleanRut: cleaned,
      digitoVerificador: providedDv,
      ...(isValid ? {} : { errorMessage: `Dígito verificador incorrecto. Debería ser: ${calculatedDv}` })
    };
  ...
  const generateValidRuts = useCallback(async (options: RutGenerationOptions): Promise<GeneratedRut[]> => {
...
export const useUfConsultor = () => {
  const [consultaResult, setConsultaResult] = useState<UfConsultaResult | null>(null);
  ...
  const consultarUF = useCallback(async (fecha: string): Promise<UfConsultaResult> => {
    setIsLoading(true);
    ...
    try {
      ...
      const variation = Math.sin(dayOfYear / 365 * 2 * Math.PI) * 500 + (Math.random() - 0.5) * 100;
  ...
  const calcularConversionUF = useCallback((
    ...
    return new Promise((resolve) => {
      setTimeout(() => {
        try {
          const valorUf = inputs.valorUfActual || consultaResult?.valor || 37250;
          ...
          if (type === 'toPesos') {
            ...
            result = {
              ...
              fechaConsulta: (consultaResult?.fecha || new Date().toISOString().split('T')[0]) as string
            };
          } else {
  ...
  return {
    ...
    clearCalculator: () => setCalculatorState({ loading: false, error: null, result: null })
  };
};
...
export const useUtmConsultor = () => {
  ...
  const consultarUTM = useCallback(async (mes: number, ano: number): Promise<UtmConsultaResult> => {
    setIsLoading(true);
  ...
  const calcularConversionUTM = useCallback((
    ...
    return new Promise((resolve) => {
      setTimeout(() => {
        try {
          const valorUtm = inputs.valorUtmActual || consultaResult?.valor || 60000;
          ...
          if (type === 'toPesos') {
            ...
            result = {
              ...
              periodoConsulta: consultaResult?.periodo || `${new Date().toLocaleDateString('es-CL', { month: 'long' })} ${new Date().getFullYear()}`
            };
          } else {
...
export const usePeriodosRapidos = () => {
  ...
  const getPeriodoRapidoUTM = useCallback((key: string): { mes: number; ano: number } => {
    const today = new Date();
...
export const useNumberFormatter = () => {
  ...
  const formatNumber = useCallback((amount: number, decimals: number = 0): string => {
    return amount.toLocaleString('es-CL', {
...
export const useDateUtils = () => {
  ...
  const formatDate = useCallback((date: string, locale: string = 'es-CL'): string => {
