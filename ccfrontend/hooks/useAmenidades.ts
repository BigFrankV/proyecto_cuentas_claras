...
export const useAmenidades = () => {
  ...
  const fetchAmenidades = useCallback(async (filters?: AmenidadFilters) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Error al cargar las amenidades');
    } finally {
      setLoading(false);
    }
  }, [getApi]);
  ...
  const createAmenidad = useCallback(async (data: AmenidadFormData): Promise<boolean> => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Error al crear la amenidad');
      return false;
    } finally {
      setLoading(false);
    }
  ...
  const updateAmenidad = useCallback(async (id: string, data: Partial<AmenidadFormData>): Promise<boolean> => {
    if (typeof window === 'undefined') {return false;}
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Error al actualizar la amenidad');
      return false;
    } finally {
      setLoading(false);
    }
  ...
  const deleteAmenidad = useCallback(async (id: string): Promise<boolean> => {
    if (typeof window === 'undefined') {return false;}
    ...
    try {
      ...
      if (success) {
        setAmenidades(prev => prev.filter(amenidad => amenidad.id !== Number(id)));
      }
      return success;
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Error al eliminar la amenidad');
      return false;
    } finally {
      setLoading(false);
    }
  ...
  const getAmenidadById = useCallback(async (id: string): Promise<Amenidad | null> => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Error al cargar la amenidad');
      return null;
    } finally {
      setLoading(false);
    }
  ...
  const getAmenidadDetalle = useCallback(async (id: string): Promise<Amenidad | null> => {
    if (typeof window === 'undefined') {return null;}
  ...
  const getAmenidadesByComunidad = useCallback(async (comunidadId: number): Promise<Amenidad[]> => {
    if (typeof window === 'undefined') {return [];}
  ...
  // Buscar amenidades
  const searchAmenidades = useCallback(async (filters: AmenidadFilters): Promise<Amenidad[]> => {
    if (typeof window === 'undefined') {return [];}
...
export const useReservasAmenidades = (amenidadId?: string) => {
  ...
  const createReserva = useCallback(async (amenidadId: string, data: ReservaAmenidadFormData): Promise<boolean> => {
    if (typeof window === 'undefined') {return false;}
  ...
  const updateReserva = useCallback(async (id: string, data: Partial<ReservaAmenidadFormData>): Promise<boolean> => {
    if (typeof window === 'undefined') {return false;}
  ...
  const deleteReserva = useCallback(async (id: string): Promise<boolean> => {
    if (typeof window === 'undefined') {return false;}
    ...
    try {
      ...
      const success = await api.delete(id);
      if (success) {
        setReservas(prev => prev.filter(reserva => reserva.id !== Number(id)));
      }
      return success;
    } catch (err) {
  ...
  // Confirmar reserva
  const confirmReserva = useCallback(async (id: string): Promise<boolean> => {
    if (typeof window === 'undefined') {return false;}
  ...
  // Cancelar reserva
  const cancelReserva = useCallback(async (id: string): Promise<boolean> => {
    if (typeof window === 'undefined') {return false;}
