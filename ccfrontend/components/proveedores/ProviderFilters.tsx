...
export interface ProviderFiltersProps {
  ...
  onChange: (payload: Partial<{ search: string; status: string; comunidadId: number | null }>) => void;
  onClear?: () => void;
}

export default function ProviderFilters({ search, status, comunidadId, comunidades, onChange, onClear }: ProviderFiltersProps) {
  return (
    <Form className="mb-3">
      <Row className="g-2 align-items-center">
        <Col md={6} xs={12}>
          <InputGroup>
            ...
            <Form.Control placeholder="Buscar proveedor..." value={search} onChange={e => onChange({ search: e.target.value })} />
          </InputGroup>
        </Col>

        <Col md={3} xs={6}>
          <Form.Select value={status} onChange={e => onChange({ status: e.target.value })}>
