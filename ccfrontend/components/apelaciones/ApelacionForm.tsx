...
export default function ApelacionForm({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!multa_id || motivo.trim().length < 6) {
      return setError('Multa ID válido y motivo (>=6 caracteres) requeridos');
    }
    setLoading(true);
    try {
    ...
    <form onSubmit={handleSubmit}>
      ...
      <div className='mb-3'>
        ...
        <input
          ...
          value={multaId}
          onChange={e => setMultaId(e.target.value)}
        />
      </div>
      <div className='mb-3'>
      ...
      <div className='mb-3'>
        ...
        <small className='text-muted'>
          Sube archivos en el módulo de archivos y pega aquí el JSON con URLs
          (opcional)
        </small>
      </div>
      <button className='btn btn-primary' type='submit' disabled={loading}>
        {loading ? 'Creando...' : 'Crear apelación'}
      </button>
    </form>
  );
}
