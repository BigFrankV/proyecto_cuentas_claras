...
const sampleAppeals = [
    {
      ...
      description: 'El residente solicita reconsideración debido a circunstancias atenuantes no consideradas inicialmente.',
    ...
    },
    {
      ...
      description: 'Se adjunta permiso médico que justifica el estacionamiento en zona prohibida.',
    ...
    },
    {
      ...
      description: 'La multa fue aplicada incorrectamente ya que la mascota estaba bajo supervisión.',
    ...
    },
    {
      ...
      description: 'La evidencia presentada no es suficiente para justificar la apelación.',
      ...
      <div className='container-fluid p-4'>
        ...
        <div className='filters-panel mb-4'>
          <div className='d-flex flex-wrap align-items-center'>
            ...
            <a href='#' className={`filter-chip ${filter === 'under_review' ? 'active' : ''}`} onClick={() => setFilter('under_review')}>
              En Revisión
            </a>
            <a href='#' className={`filter-chip ${filter === 'approved' ? 'active' : ''}`} onClick={() => setFilter('approved')}>
              Aprobadas
            </a>
            <a href='#' className={`filter-chip ${filter === 'rejected' ? 'active' : ''}`} onClick={() => setFilter('rejected')}>
              Rechazadas
            </a>
          </div>
        </div>
        ...
        {selectedAppeals.length > 0 && (
          <div className='bulk-actions show mb-4'>
            <div className='d-flex justify-content-between align-items-center'>
              ...
              <div>
                <button className='btn btn-light me-2' onClick={() => {/* bulk approve */}}>
                  <i className='material-icons me-1'>check_circle</i>
                  Aprobar seleccionadas
                </button>
                <button className='btn btn-light' onClick={() => {/* bulk reject */}}>
            ...
            <tbody>
              {filteredAppeals.map((appeal) => (
                <tr key={appeal.id}>
                  ...
                  <td>
                    <Link href={`/apelacion-detalle/${appeal.id}`} className='text-decoration-none fw-bold'>
                      {appeal.id}
                    </Link>
                  </td>
                  <td>
                    <Link href={`/multa-detalle/${appeal.fineId}`} className='text-decoration-none'>
                      {appeal.fineId}
                    </Link>
                  </td>
                  ...
                  <td>
                    <span className={`appeal-status-badge ${getStatusBadge(appeal.status)}`}>
                  ...
                  <td>
                    <div className='btn-group'>
                      <button className='btn btn-sm btn-outline-primary' data-bs-toggle='modal' data-bs-target='#reviewModal'>
                        <i className='material-icons'>visibility</i>
                      </button>
                      {appeal.status === 'pending' && (
                        <>
                          <button className='btn btn-sm btn-outline-success' onClick={() => alert('Apelación aprobada')}>
                            <i className='material-icons'>check</i>
                          </button>
                          <button className='btn btn-sm btn-outline-danger' onClick={() => alert('Apelación rechazada')}>
          ...
          {filteredAppeals.map((appeal) => (
            <div key={appeal.id} className={`appeal-card ${appeal.status}`}>
              <div className='appeal-header'>
                <div>
                  <div className='appeal-number'>
                    <Link href={`/apelacion-detalle/${appeal.id}`} className='text-decoration-none'>
                      {appeal.id}
                    </Link>
                  </div>
                ...
                <div className='text-end'>
                  <div className='appeal-amount'>${appeal.amount.toLocaleString()}</div>
                  <span className={`appeal-status-badge ${getStatusBadge(appeal.status)}`}>
                    {getStatusText(appeal.status)}
                  </span>
                </div>
              ...
              <div className='d-flex justify-content-between align-items-center mt-3'>
                <span className='appeal-date'>Enviada: {appeal.submittedDate}</span>
                <div className='btn-group'>
