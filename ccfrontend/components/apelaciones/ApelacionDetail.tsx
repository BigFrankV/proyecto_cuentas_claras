...
export default function ApelacionDetail({ apelacion, onResolved, onUpdated }: { apelacion: any, onResolved?: Function, onUpdated?: Function }) {
  ...
  async function handleResolve(accion: 'aceptar'|'rechazar') {
    ...
    try {
      ...
      if (onResolved) onResolved(apelacion.id);
    } catch (err) {
  ...
  return (
    <div className="apelacion-detail card p-3">
      ...
      <p><strong>Estado:</strong> <span className={`badge ${apelacion.estado==='pendiente'?'bg-warning text-dark':apelacion.estado==='aprobada'?'bg-success':'bg-danger'}`}>{(apelacion.estado||'—').toUpperCase()}</span></p>
      ...
      <div className="mt-3">
        ...
        {!isManager() && (user && (user.id === apelacion.usuario_id || user.persona_id === apelacion.persona_id)) && apelacion.estado === 'pendiente' && (
