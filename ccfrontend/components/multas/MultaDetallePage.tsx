...
const parseDateSafe = (d?: string | Date | null): Date | null => {
  ...
  if (mDtLocal) {
    ...
    return new Date(Number(y), Number(mo) - 1, Number(day), Number(hh), Number(mm), Number(ss));
  }
...
const MultaDetallePage: React.FC<MultaDetallePageProps> = ({ multa, historial = [] }) => {
  const [activeTab, setActiveTab] = useState<'overview' | 'evidence' | 'payments' | 'appeals' | 'communications'>('overview');
  const [payments, setPayments] = useState<any[] | null>(null);
  const [documents, setDocuments] = useState<any[] | null>(null);
  const [appeals, setAppeals] = useState<any[] | null>(null);
  const [comms, setComms] = useState<any[] | null>(historial.length ? historial : null);
  ...
  const fechaCreada = parseDateSafe(multa.created_at) ?? parseDateSafe(multa.fecha) ?? new Date();
  // Si tienes fecha_vencimiento en BD úsala; si no, tomar `fecha` como vencimiento.
  const fechaVencBD = parseDateSafe(multa.fecha_vencimiento) ?? parseDateSafe(multa.fecha);
  const fechaVencimiento = fechaVencBD ?? new Date(fechaCreada.getTime() + 15 * 24 * 60 * 60 * 1000);
    ...
    const load = async () => {
      ...
      try {
        if (activeTab === 'payments' && payments === null) {
          const h = (historial && historial.length) ? historial : (await multasService.getHistorial(Number(multa.id)));
          const histArr = Array.isArray(h) ? h : (h?.data ?? []);
          if (!mounted) return;
          setPayments((histArr || []).filter((x: any) => String(x.accion).toLowerCase().includes('pago')));
        }
        ...
        if (activeTab === 'appeals' && appeals === null) {
          try {
            ...
            const histArr = Array.isArray(h) ? h : (h?.data ?? []);
            if (!mounted) return;
            setAppeals((histArr || []).filter((x: any) => String(x.accion).toLowerCase().includes('apel')));
          }
        }
      } catch (e) {
  ...
  const handleRegistrarPago = async () => {
    if (!multa || !paymentFormRef.current) return;
    const form = paymentFormRef.current;
    const fd = new FormData(form);
    const payload = {
      monto: Number(fd.get('monto')) || undefined,
      fecha_pago: fd.get('fecha_pago') ? String(fd.get('fecha_pago')) : undefined,
      metodo_pago: fd.get('metodo_pago') ? String(fd.get('metodo_pago')) : undefined,
      referencia: fd.get('referencia') ? String(fd.get('referencia')) : undefined,
      observaciones: fd.get('observaciones') ? String(fd.get('observaciones')) : undefined
    };
  ...
  const handleCrearApelacion = async () => {
    if (!multa || !appealFormRef.current) return;
    setAppealError(null);
    ...
    try {
      setLoadingTab(true);
      const createResp = await multasService.crearApelacion(Number(multa.id), payload);

      // Si el backend devuelve { success: false, details: [...] }
      if (createResp && createResp.success === false) {
        const msgs = (createResp.details || []).map((d: any) => d.msg).filter(Boolean).join('; ');
        setAppealError(msgs || String(createResp.error || 'Error creando apelación'));
        return;
      }
          ...
          <div className='d-flex justify-content-between align-items-center'>
            <div>
              ...
              <small className='text-muted'>
                Emitida {formatDate(fechaCreada)} • Vence {formatDate(fechaVencimiento)}
              </small>
            </div>
      ...
      <div className='row'>
        <div className='col-lg-8'>
          <ul className='nav nav-tabs' id='fineDetailTabs' role='tablist'>
            ...
            <li className='nav-item'>
              <button className={`nav-link ${activeTab === 'evidence' ? 'active' : ''}`} onClick={() => setActiveTab('evidence')}>
                <span className='material-icons me-2'>attach_file</span>Evidencia
              </button>
            </li>
            <li className='nav-item'>
            ...
            <li className='nav-item'>
              <button className={`nav-link ${activeTab === 'communications' ? 'active' : ''}`} onClick={() => setActiveTab('communications')}>
                <span className='material-icons me-2'>message</span>Comunicaciones
              </button>
            </li>
          </ul>
          ...
          <div className='tab-content mt-0' style={{ marginTop: 0 }}>
              ...
              <div className='row'>
                <div className='col-md-6'>
                  ...
                  <div className='info-item'>
                    ...
                    <span className='info-value'>{formatDate(fechaCreada)}</span>
                  </div>
                  <div className='info-item'>
                  ...
                  <div className='info-item'>
                    ...
                    <span className='info-value'>{multa.tipo_infraccion ?? multa.motivo}</span>
                  </div>
                </div>
                <div className='col-md-6'>
                  ...
                  <div className='info-item'>
                    ...
                    <span className='info-value'>${(multa.monto ?? 0).toLocaleString?.() ?? multa.monto}</span>
                  </div>
                  <div className='info-item'>
            ...
            <div className={`tab-pane fade ${activeTab === 'evidence' ? 'show active' : ''}`}>
              <h6 className='mb-3'>Evidencia Adjunta</h6>
              {loadingTab ? <div>Cargando evidencia...</div> : (documents && documents.length === 0) ? (
                <div className='alert alert-secondary'>Sin evidencia. <button className='btn btn-sm btn-outline-primary ms-2' onClick={() => alert('Subir evidencia')}>Subir evidencia</button></div>
              ) : (
                <ul className='list-group'>
                  {documents?.map((d: any, i: number) => (
                    <li key={d.id ?? i} className='list-group-item d-flex justify-content-between align-items-center'>
                      <div>
                        <strong>{d.nombre_archivo ?? d.file_name}</strong>
                        <div className='small text-muted'>{d.descripcion ?? ''}</div>
                      </div>
                      <a href={d.ruta_archivo ?? d.url} target='_blank' rel='noreferrer' className='btn btn-sm btn-outline-secondary'>Ver</a>
                    </li>
                  ))}
                </ul>
              )}
            ...
            <div className={`tab-pane fade ${activeTab === 'payments' ? 'show active' : ''}`}>
              <div className='d-flex justify-content-between align-items-center mb-3'>
                ...
                <button className='btn btn-primary btn-sm' data-bs-toggle='modal' data-bs-target='#paymentModal'>Registrar Pago</button>
              </div>
              {loadingTab ? <div>Cargando pagos...</div> : (payments && payments.length === 0) ? (
                <div className='alert alert-info'>Sin pagos registrados. <button className='btn btn-sm btn-primary ms-2' onClick={() => alert('Registrar pago')}>Registrar Pago</button></div>
              ) : (
            ...
            <div className={`tab-pane fade ${activeTab === 'appeals' ? 'show active' : ''}`}>
              <h6>Apelaciones</h6>
              {loadingTab ? <div>Cargando apelaciones...</div> : (appeals && appeals.length === 0) ? (
                <div className='alert alert-secondary'>Sin apelaciones. <button className='btn btn-sm btn-outline-primary ms-2' data-bs-toggle='modal' data-bs-target='#appealModal'>Crear Apelación</button></div>
              ) : (
                <ul className='list-group'>
                    ...
                    <li key={a.id ?? i} className='list-group-item'>
                      <div className='d-flex justify-content-between'>
                        <div>
                          ...
                          <div className='small text-muted'>{a.resolucion ?? ''}</div>
                        </div>
              ...
              <div className='modal-dialog'>
                <div className='modal-content'>
                  <div className='modal-header'>
                    <h5 className='modal-title'>Crear Apelación - Multa #{multa.numero}</h5>
                  ...
                  <div className='modal-body'>
                    <form ref={appealFormRef} id='appealForm'>
                      <div className='mb-3'>
                        ...
                        <textarea name='motivo' className='form-control' rows={4} required minLength={20} />
            ...
            <div className={`tab-pane fade ${activeTab === 'communications' ? 'show active' : ''}`}>
              <h6>Comunicaciones / Historial</h6>
              {loadingTab ? <div>Cargando historial...</div> : (comms && comms.length === 0) ? (
                <div className='alert alert-secondary'>Sin comunicaciones registradas.</div>
              ) : (
                comms?.map((c: any, i: number) => (
                  <div key={c.id ?? i} className='mb-3'>
                    <div className='d-flex justify-content-between'>
                      <div><strong>{c.accion}</strong></div>
                      <small className='text-muted'>{formatDate(c.created_at)}</small>
                    </div>
                    <div className='text-muted small'>{c.observaciones ?? c.descripcion}</div>
                  </div>
                ))
              )}
            </div>
        ...
        <div className='col-lg-4'>
          <div className='info-sidebar'>
            <div className='info-card'>
              <div className='info-card-title'><span className='material-icons'>apartment</span> Información de la Unidad</div>
              <div className='info-item'><span className='info-label'>Unidad:</span><span className='info-value'>{multa.torre_nombre} - {multa.unidad_numero}</span></div>
              <div className='info-item'><span className='info-label'>Propietario:</span><span className='info-value'>{multa.propietario_nombre ?? '—'}</span></div>
              <div className='info-item'><span className='info-label'>Email:</span><span className='info-value'>{multa.propietario_email ?? '—'}</span></div>
              <div className='mt-3'><button className='btn btn-outline-primary btn-sm w-100' onClick={() => alert('Ver detalle unidad')}>Ver Detalle de Unidad</button></div>
            </div>

            <div className='info-card'>
              <div className='info-card-title'><span className='material-icons'>analytics</span> Historial de Multas</div>
              <div className='info-item'><span className='info-label'>Última Multa:</span><span className='info-value'>{formatDate(multa.fecha)}</span></div>
            </div>

            <div className='info-card'>
              <div className='info-card-title'><span className='material-icons'>timeline</span> Actividades Recientes</div>
              ...
              <form ref={paymentFormRef} id='paymentForm'>
                ...
                <div className='mb-3'>
                  <div className='form-check'>
                    <input className='form-check-input' name='enviar_confirmacion' type='checkbox' defaultChecked />
              ...
              <form>
                <div className='row'>
                  <div className='col-md-6 mb-3'>
                    ...
                    <input type='date' className='form-control' defaultValue={formatDateInput(multa.fecha)} />
                  </div>
                  <div className='col-md-6 mb-3'>
