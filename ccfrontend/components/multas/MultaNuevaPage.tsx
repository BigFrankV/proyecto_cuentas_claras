...
const MultaNuevaPage: React.FC = () => {
  ...
  const [selectedViolationType, setSelectedViolationType] = useState<number | null>(null);
  const [selectedUnitCode, setSelectedUnitCode] = useState<string | null>(null);
  const [selectedUnitId, setSelectedUnitId] = useState<number | null>(null);
  const [selectedUnitComunidadId, setSelectedUnitComunidadId] = useState<number | null>(null);
  ...
  const [selectedComunidadGlobal, setSelectedComunidadGlobal] = useState<number | null>(null);
  ...
  const filteredUnits = units.filter(
    unit =>
      ...
      (unit.owner || unit.propietario || '').toLowerCase().includes(unitSearch.toLowerCase())
  );
    ...
    (async () => {
      try {
        ...
        const base = (process.env.NEXT_PUBLIC_API_URL && process.env.NEXT_PUBLIC_API_URL !== '')
          ...
          : 'http://localhost:3001';
        const resp = await fetchWithAuth(`${base}/multas/comunidades/mis`, { method: 'GET' });
        if (!resp.ok) {
          console.warn('No se pudieron cargar comunidades (status):', resp.status);
          return;
        }
  ...
  useEffect(() => {
    let timer = setTimeout(async () => {
      try {
        ...
        const data = await multasService.obtenerUnidadesAutocompletar(unitSearch);
  ...
  const renderComunidadSelector = () => {
    if (comunidadesLoading) return <div className='mb-3'>Cargando comunidades...</div>;
    if (!comunidades || comunidades.length <= 1) {
      if (comunidades && comunidades.length === 1) {
        return <div className='mb-3'><strong>Comunidad:</strong> {comunidades[0].razon_social}</div>;
      }
      return null;
    }
    return (
      <div className='mb-3'>
        ...
        <select className='form-select' value={selectedComunidadGlobal ?? ''} onChange={e => setSelectedComunidadGlobal(e.target.value ? Number(e.target.value) : null)}>
  ...
  const handleSelectUnit = async (unit: { id: number; codigo: string; comunidad_id?: number }) => {
    ...
    try {
      if (unit.comunidad_id) {
        const tipos = await multasService.obtenerTipos(Number(unit.comunidad_id));
        setViolationTypes((tipos || []).map((t: any, i: number) => ({
          id: t.id,
          title: t.nombre,
          description: t.descripcion,
          amount: Number(t.monto_default || 0),
          icon: t.icono || ['warning', 'gavel', 'pets', 'cleaning_services'][i % 4],
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setViolationTypes([
          { id: 1, title: 'Ruido excesivo', description: 'Violación de normas de ruido', amount: 50000, icon: 'volume_up', color: '#ff6b6b' },
          { id: 2, title: 'Estacionamiento indebido', description: 'Vehículo mal estacionado', amount: 30000, icon: 'local_parking', color: '#6f42c1' },
          { id: 3, title: 'Mascotas sin control', description: 'Mascotas sin correa o supervisión', amount: 25000, icon: 'pets', color: '#0d6efd' },
          { id: 4, title: 'Falta de limpieza', description: 'Áreas comunes sucias', amount: 20000, icon: 'cleaning_services', color: '#198754' }
        ]);
      }
    })();
  }, []);
  ...
  const handleSubmit = async () => {
    if (!validateStep(3)) {
      window.alert('Completa todos los campos obligatorios antes de crear la multa.');
      return;
    }
    if (!selectedUnitId) {
    ...
    const tipoIdNum = (selectedViolation as any)?.id ? Number((selectedViolation as any).id) : undefined;
    const payload: any = {
    ...
    <Layout title='Nueva Multa'>
      <div className='container-fluid p-4'>
              ...
              <div key={v.id} className='col-md-6 col-lg-3 mb-4'>
                ...
                >
                  <div className='d-flex align-items-center'>
                    <div className='me-3 d-flex align-items-center' style={{ minWidth: 48 }}>
                      <span
                    ...
                    <div className='flex-grow-1'>
                      <h5 className='mb-1' style={{ color: v.color }}>{v.title}</h5>
                      <p className='small mb-1'>{v.description}</p>
                      <p className='mb-0'><strong>{v.amount.toLocaleString('es-CL')} CLP</strong></p>
                    </div>
                  </div>
                </div>
              </div>
        ...
        <div className={`wizard-section ${currentStep === 2 ? 'active' : ''}`}>
          ...
          <div className='mb-4'>
            <input
              ...
              placeholder={comunidades && comunidades.length > 1 && !selectedComunidadGlobal ? 'Seleccione comunidad primero...' : 'Buscar por número de unidad o nombre del propietario...'}
              value={unitSearch}
                ...
                >
                  <div className='d-flex w-100 justify-content-between'>
                    <div>
                      ...
                      <small className='text-muted'>{u.owner || u.propietario || '-'}</small>
                    </div>
                    <div>
          ...
          <div className='row'>
            <div className='col-md-8'>
              <div className='form-section mb-3'>
                ...
                <div className='row'>
                  <div className='col-md-6 mb-3'>
                    ...
                    <input type='date' className='form-control' value={fechaInfraccion} onChange={e => setFechaInfraccion(e.target.value)} />
                  </div>
                  <div className='col-md-6 mb-3'>
                  ...
                  <div className='col-md-6 mb-3'>
                    ...
                    <input type='number' className='form-control' value={monto} onChange={e => setMonto(e.target.value === '' ? '' : Number(e.target.value))} min={0} />
                  </div>
                  <div className='col-md-6 mb-3'>
                    <label className='form-label'>Prioridad</label>
                    <select className='form-select' value={prioridad} onChange={e => setPrioridad(mapPriority(e.target.value) as any)}>
              ...
              <div className='form-section'>
                ...
                >
                  <i className='material-icons mb-2' style={{ fontSize: 36 }}>cloud_upload</i>
                ...
                <div id='uploadedFiles' className='mt-3'>
                  {uploadedFiles.map((file, index) => (
                    <div key={index} className='uploaded-file d-flex align-items-center justify-content-between border rounded p-2 mb-2'>
                      <div className='d-flex align-items-center'>
                        <div className='file-icon me-3'><i className='material-icons'>insert_drive_file</i></div>
                        <div>
                          <div className='file-name'>{file.name}</div>
                          <div className='file-size text-muted'>{(file.size / 1024 / 1024).toFixed(2)} MB</div>
                        </div>
                      </div>
            ...
            <div className='col-md-4'>
              <div className='form-section'>
                ...
                <div className='summary-item mb-2'>
                  <div className='summary-label text-muted'>Tipo de infracción</div>
                ...
                <div className='summary-item mb-2'>
                  ...
                  <div className='summary-value'>{monto ? `${Number(monto).toLocaleString('es-CL')} CLP` : '-'}</div>
                </div>
                <div className='summary-item mb-2'>
                  <div className='summary-label text-muted'>Archivos</div>
                  <div className='summary-value'>{uploadedFiles.length} adjuntos</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className='row'>
            <div className='col-md-8'>
              <div id='reviewContent'>
                ...
                <dl>
                  <dt>Tipo</dt><dd>{selectedViolation?.title}</dd>
                  <dt>Unidad</dt><dd>{selectedUnitCode}</dd>
                  <dt>Monto</dt><dd>{monto ? `${Number(monto).toLocaleString('es-CL')} CLP` : '-'}</dd>
                  <dt>Fecha</dt><dd>{fechaInfraccion || '-'}</dd>
        ...
        <div className='navigation-buttons mt-4 d-flex justify-content-between'>
          <div>
            <button className='btn btn-outline-secondary me-2' onClick={() => changeStep(-1)} disabled={currentStep === 1}>
              <i className='material-icons me-2'>arrow_back</i>
              Anterior
            </button>
            {currentStep < 4 && (
              <button
                ...
                onClick={() => { if (validateStep(currentStep)) changeStep(1); else window.alert('Completa los campos obligatorios en este paso.'); }}
              >
                Siguiente
