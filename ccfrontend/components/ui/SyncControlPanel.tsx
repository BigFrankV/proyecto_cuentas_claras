...
const SyncControlPanel: React.FC<SyncControlPanelProps> = ({ 
  ...
  const [syncStatus, setSyncStatus] = useState<indicadoresAPI.SyncStatusResponse | null>(null);
  ...
  const handleInitialize = async () => {
    ...
    try {
      ...
      if (result.success) {
        setMessage(result.message || 'Inicialización de datos históricos completada');
        setMessageType('success');
  ...
  return (
    <div className={`card ${className}`}>
      ...
      <div className="card-body">
        {/* Mensaje de estado */}
        {message && (
          <div className={`alert alert-${messageType === 'error' ? 'danger' : messageType === 'success' ? 'success' : 'info'} alert-dismissible fade show`} role="alert">
            <i className={`material-icons me-2 ${messageType === 'error' ? 'error' : messageType === 'success' ? 'check_circle' : 'info'}`}></i>
            {message}
            <button 
              type="button" 
              className="btn-close" 
          ...
          <>
            ...
            <div className="row mb-3">
              <div className="col-md-6">
                <div className="d-flex align-items-center mb-2">
                  <span className={`badge me-2 ${syncStatus.isRunning ? 'bg-warning' : 'bg-success'}`}>
            ...
            <div className="mt-3">
              <small className="text-muted">
                <i className="material-icons me-1" style={{ fontSize: '14px' }}>info</i>
                La sincronización automática se ejecuta diariamente a las 08:00 AM.
                Use "Inicializar" solo si es la primera vez que configura el sistema.
              </small>
            </div>
          </>
        ) : (
          <div className="text-center text-muted py-3">
            <i className="material-icons mb-2" style={{ fontSize: '2rem' }}>error_outline</i>
