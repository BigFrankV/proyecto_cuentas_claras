...
const ComunidadTable: React.FC<ComunidadTableProps> = ({ comunidades, onEdit, onDelete }) => {
  return (
        ...
        <table className="table table-hover mb-0">
          <thead className="table-light">
            <tr>
              ...
              <th scope="col" className="actions-cell">Acciones</th>
            </tr>
          </thead>
          <tbody>
            {comunidades.map((comunidad) => {
              const ocupacionPorcentaje = (comunidad.unidadesOcupadas / comunidad.totalUnidades) * 100;
                ...
                <tr key={comunidad.id}>
                  <td>
                    <div className="d-flex align-items-center">
                      <img
                        src={comunidad.imagen || 'https://images.unsplash.com/photo-1486406146926-c627a92ad1ab?ixlib=rb-1.2.1&auto=format&fit=crop&w=100&q=80'}
                      ...
                      <div>
                        ...
                        <small className="text-muted">{comunidad.direccion}</small>
                      </div>
                    </div>
                  </td>
                  <td>
                    <div className="d-flex align-items-center">
                      <span className="material-icons me-1 text-muted" style={{ fontSize: '18px' }}>
                  ...
                  <td>
                    <div className="text-center">
                      ...
                      <small className="text-muted">{comunidad.unidadesOcupadas} ocupadas</small>
                    </div>
                  </td>
                  <td>
                    <div className="d-flex align-items-center">
                      ...
                      <small className="text-muted">{ocupacionPorcentaje.toFixed(0)}%</small>
                    </div>
                  </td>
                  <td>
                    <span className={`fw-bold ${
                      comunidad.morosidad > 10 ? 'text-danger' : 
                      comunidad.morosidad > 5 ? 'text-warning' : 'text-success'
                    }`}>
                      {comunidadesService.formatPercentage(comunidad.morosidad)}
                    </span>
                  </td>
                  <td>
                    <span className={`fw-bold ${comunidad.saldoPendiente > 0 ? 'text-warning' : 'text-success'}`}>
                      {comunidadesService.formatCurrency(comunidad.saldoPendiente)}
                    </span>
                  </td>
                  <td>
                    <div className="d-flex align-items-center">
                      <span className="material-icons me-1 text-muted" style={{ fontSize: '16px' }}>person</span>
