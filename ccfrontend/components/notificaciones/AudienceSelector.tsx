...
const mockUserGroups: UserGroup[] = [
  { id: 'owners', name: 'Propietarios', count: 245, description: 'Todos los propietarios' },
  { id: 'tenants', name: 'Inquilinos', count: 134, description: 'Personas que arriendan' },
  { id: 'council', name: 'Consejo', count: 8, description: 'Miembros del consejo' },
  { id: 'admins', name: 'Administradores', count: 3, description: 'Personal administrativo' },
  { id: 'debtors', name: 'Morosos', count: 23, description: 'Con deuda pendiente' },
  { id: 'current', name: 'Al día', count: 222, description: 'Sin deudas pendientes' }
];
...
export default function AudienceSelector({ 
  ...
  const [selectedBuildings, setSelectedBuildings] = useState<string[]>(value.buildingIds || []);
  const [selectedUnits, setSelectedUnits] = useState<string[]>(value.unitIds || []);
  const [selectedGroups, setSelectedGroups] = useState<string[]>(value.userGroupIds || []);
  ...
  const handleUnitChange = (unitNumbers: string) => {
    const units = unitNumbers.split(',').map(u => u.trim()).filter(u => u);
  ...
  return (
    <div className={`audience-selector ${className}`}>
      ...
      <div className='audience-options mb-4'>
        ...
        <div className='form-check mb-3'>
          ...
          <label className='form-check-label' htmlFor='audience-building'>
            ...
            {selectedBuildings.length > 0 && (
              <div className='small text-info'>~{getEstimatedCount()} personas</div>
            )}
          </label>
        </div>
        ...
        <div className='form-check mb-3'>
          ...
          <label className='form-check-label' htmlFor='audience-unit'>
            ...
            {selectedUnits.length > 0 && (
              <div className='small text-info'>{getEstimatedCount()} unidades</div>
            )}
          </label>
        </div>
        ...
        <div className='form-check mb-3'>
          ...
          <label className='form-check-label' htmlFor='audience-custom'>
            ...
            <div className='small text-muted'>Seleccionar grupos específicos</div>
      ...
      {/* Building Selector */}
      {value.type === 'building' && (
        <div className='building-selector mb-3' style={{
            ...
            {mockBuildings.map((building) => (
              <div key={building.id} className='col-md-4'>
                <div className='form-check'>
                  ...
                  />
                  <label className='form-check-label w-100' htmlFor={`building-${building.id}`}>
                    <div className='building-option' style={{
                      backgroundColor: selectedBuildings.includes(building.id) ? '#e3f2fd' : '#fff',
                      border: selectedBuildings.includes(building.id) ? '1px solid #2196f3' : '1px solid #dee2e6',
                      borderRadius: '0.375rem',
                      padding: '0.75rem',
                      cursor: 'pointer'
                    }}>
                      <div className='fw-semibold'>{building.name}</div>
                      <div className='small text-muted'>{building.unitCount} unidades</div>
                    </div>
                  </label>
                </div>
              </div>
      ...
      {/* Unit Selector */}
      {value.type === 'unit' && (
        <div className='unit-selector mb-3' style={{
          backgroundColor: '#f8f9fa',
          borderRadius: 'var(--radius)',
          padding: '1rem',
          border: '1px solid #e9ecef'
          ...
          <div className='mb-3'>
            ...
            <div className='form-text'>
              Escribe los números de unidad separados por comas. Puedes usar formatos como: 101, A-205, B-312
            </div>
          </div>
          ...
          <div className='quick-unit-selection'>
            ...
            <div className='d-flex flex-wrap gap-2'>
              <button
                ...
                onClick={() => handleUnitChange('101,102,103,104,105,106,107,108,109,110')}
              >
                Primer piso
              </button>
              <button
                type='button'
                className='btn btn-outline-secondary btn-sm'
                onClick={() => handleUnitChange('201,202,203,204,205,206,207,208,209,210')}
              >
                Segundo piso
              </button>
              <button
      ...
      {value.type === 'custom' && (
        <div className='custom-selector mb-3' style={{
          backgroundColor: '#f8f9fa',
          borderRadius: 'var(--radius)',
          padding: '1rem',
          border: '1px solid #e9ecef'
            ...
            {mockUserGroups.map((group) => (
              <div key={group.id} className='col-md-6'>
                <div className='form-check'>
                  ...
                  />
                  <label className='form-check-label w-100' htmlFor={`group-${group.id}`}>
                    <div className='group-option' style={{
                      backgroundColor: selectedGroups.includes(group.id) ? '#e8f5e9' : '#fff',
                      border: selectedGroups.includes(group.id) ? '1px solid #4caf50' : '1px solid #dee2e6',
                      borderRadius: '0.375rem',
                      padding: '0.75rem',
                      cursor: 'pointer'
                    }}>
                      <div className='d-flex justify-content-between align-items-start'>
                        ...
                        <div className='text-primary fw-semibold'>{group.count}</div>
                      </div>
                    </div>
                  </label>
                </div>
      ...
      {/* Summary */}
      <div className='audience-summary' style={{
        ...
        <div className='d-flex align-items-center'>
          ...
          <div>
            ...
            <div className='small text-muted'>
              ...
              {value.type === 'building' && `${selectedBuildings.length} edificio(s) seleccionado(s)`}
              {value.type === 'unit' && `${selectedUnits.length} unidad(es) específica(s)`}
              {value.type === 'custom' && `${selectedGroups.length} grupo(s) personalizado(s)`}
            </div>
          </div>
        </div>
      </div>
