...
export default function CentroTable({ centros, loading, onEdit, onDelete }: CentroTableProps) {
  if (loading) {
    return <div className="py-4 text-center">Cargando centros...</div>;
  }

  if (!centros || centros.length === 0) {
    return <div className="py-4 text-center text-muted">No se encontraron centros de costo</div>;
  }
          ...
          <tr key={c.id} className="border-bottom">
            ...
            <td className="text-end">
              <div className="d-flex gap-1 justify-content-end">
                <Button variant="outline-primary" size="sm" onClick={() => onEdit(c.id)}>
