...
export default function ComprasListado() {
      ...
      const mockPurchases: Purchase[] = [
        {
          ...
          description: 'Materiales para reparación de fachada principal del edificio',
          totalAmount: 850000,
          currency: 'clp',
        ...
        },
        {
          ...
          description: 'Servicio de limpieza profunda áreas comunes del condominio',
          totalAmount: 320000,
          currency: 'clp',
        ...
        },
        {
          ...
          description: 'Reparación urgente sistema eléctrico Torre A - Falla generalizada',
          totalAmount: 780000,
          currency: 'clp',
        ...
        },
        {
          ...
          description: 'Renovación completa de jardines y áreas verdes del condominio',
          totalAmount: 450000,
          currency: 'clp',
          ...
          items: [
            {
              id: 1,
              description: 'Plantas ornamentales chilenas (lavanda, romero, copihue)',
              quantity: 25,
              unit: 'unidades',
              unitPrice: 8500,
              totalPrice: 212500,
            ...
            {
              id: 3,
              description: 'Servicio diseño paisajístico y mantención trimestral',
              quantity: 1,
              unit: 'servicio',
              unitPrice: 125000,
              totalPrice: 125000,
        ...
        },
        {
          ...
          description: 'Actualización sistema administración condominial y portería digital',
          totalAmount: 1250000,
          currency: 'clp',
  ...
  const getStatusBadge = (status: string) => {
    const badges = {
      ...
      'in-progress': { bg: 'primary', text: 'En Proceso', class: 'status-in-progress' },
      delivered: { bg: 'success', text: 'Entregado', class: 'status-delivered' },
      completed: { bg: 'success', text: 'Completado', class: 'status-completed' },
      cancelled: { bg: 'danger', text: 'Cancelado', class: 'status-cancelled' }
    };
    
    const badge = badges[status as keyof typeof badges];
    return <span className={`purchase-status-badge ${badge.class}`}>{badge.text}</span>;
  };
  ...
  const getPriorityBadge = (priority: string) => {
    ...
    return <span className={`purchase-priority-badge ${badge.class}`}>{badge.text}</span>;
  };
  ...
  const getTypeBadge = (type: string) => {
    const badges = {
      ...
      maintenance: { text: 'Mantenimiento', class: 'type-maintenance', icon: 'construction' },
      supplies: { text: 'Suministros', class: 'type-supplies', icon: 'inventory' }
    };
  ...
  const filteredPurchases = purchases.filter(purchase => {
    return (
      ...
      (filters.provider === '' || purchase.provider.name.toLowerCase().includes(filters.provider.toLowerCase())) &&
      (filters.costCenter === '' || purchase.costCenter.name.toLowerCase().includes(filters.costCenter.toLowerCase()))
    );
  });
        ...
        <div className="purchases-container">
                ...
                <Col lg={4}>
                  <div className="hero-actions">
                    <div className="action-card">
                      ...
                      <p>Crea una nueva solicitud de compra y optimiza tus procesos</p>
                      <Button 
                        variant="primary" 
                        size="lg"
                        className="hero-cta"
              ...
              <Card className="card-stat">
                <Card.Body>
                  <div className="d-flex justify-content-between align-items-center">
                    ...
                    <div className="stat-icon" style={{ backgroundColor: '#e1f5fe', color: '#0288d1' }}>
                ...
                <Card.Body>
                  <div className="d-flex justify-content-between align-items-center">
                    <div>
                      ...
                      <h3 className="mt-2 mb-0">${(stats.totalAmount / 1000000).toFixed(1)}M</h3>
                    </div>
                  ...
                  <thead>
                    <tr>
                      <th>
                        <Form.Check
                          type="checkbox"
                          checked={selectedPurchases.length === paginatedPurchases.length && paginatedPurchases.length > 0}
                      ...
                      <tr key={purchase.id} className="data-row">
                        ...
                        <td>
                          ...
                          <small className="text-muted">
                            {new Date(purchase.requestDate).toLocaleDateString()}
                          </small>
                        </td>
                        <td>
                          <div className="fw-medium">{purchase.description}</div>
                          <small className="text-muted">
                            {purchase.items.length} item(s) • {purchase.documents} doc(s)
                          </small>
                        </td>
                        ...
                        <td>
                          <div className="fw-medium">{purchase.provider.name}</div>
                          <small className="text-muted">{purchase.provider.category}</small>
                        </td>
                        <td>
                          <div className="fw-medium">{purchase.costCenter.name}</div>
                          <small className="text-muted">{purchase.costCenter.department}</small>
                        </td>
                        <td>
                        ...
                        <td>
                          <div>{new Date(purchase.requiredDate).toLocaleDateString()}</div>
                          <small className="text-muted">
                    ...
                    <div className="card-body">
                      <div className="d-flex justify-content-between align-items-start mb-3">
                        <div>
                          ...
                          <small className="text-muted">{purchase.description}</small>
                        </div>
                      ...
                      <div className="purchase-stats mb-3">
                        <Row className="text-center">
                          <Col xs={4}>
                            <div className="fw-bold">{purchase.items.length}</div>
                            <small className="text-muted">Items</small>
                          </Col>
                          <Col xs={4}>
                            <div className="fw-bold">{formatCurrency(purchase.totalAmount, purchase.currency)}</div>
                      ...
                      <div className="d-flex justify-content-between align-items-center">
                        <small className="text-muted">
                          Vence: {new Date(purchase.requiredDate).toLocaleDateString()}
                        </small>
          ...
          {totalPages > 1 && (
            <div className="d-flex justify-content-between align-items-center mt-4">
              <span className="text-muted">
                Mostrando {startIndex + 1}-{Math.min(startIndex + itemsPerPage, filteredPurchases.length)} de {filteredPurchases.length} compras
              </span>
              <nav>
          ...
          <Modal.Body>
            {selectedPurchase && (
              <>
                ...
                <p>¿Estás seguro de que deseas eliminar la compra <strong>"{selectedPurchase.number}"</strong>?</p>
                <p className="text-muted">
                  Esto también eliminará toda la información relacionada, incluyendo items y documentos.
                </p>
              </>
            )}
          </Modal.Body>
